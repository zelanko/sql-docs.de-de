---
title: Bekannte Probleme in Machine Learning-Dienste | Microsoft-Dokumentation
ms.prod: sql
...
ms.locfileid: "51704108"
---
# <a name="known-issues-in-machine-learning-services"></a>Bekannte Probleme in Machine Learning-Dienste
[!INCLUDE[appliesto-ss-xxxx-xxxx-xxx-md-winonly](../includes/appliesto-ss-xxxx-xxxx-xxx-md-winonly.md)]

Dieser Artikel beschreibt bekannte Probleme oder Einschränkungen mit Machine learning-Komponenten, die als Option in SQL Server 2016 und SQL Server 2017 bereitgestellt werden.

Die folgenden Informationen gilt für alle der folgenden, sofern nicht anders angegeben:
...
- Machine Learning-Dienste für R (Datenbankintern)
...
### <a name="unable-to-install-sql-server-machine-learning-features-on-a-domain-controller"></a>Kann nicht zum Installieren von SQL Server Machine Learning-Features auf einem Domänencontroller
...
### <a name="install-the-latest-service-release-to-ensure-compatibility-with-microsoft-r-client"></a>Installieren Sie der letzten Dienstversion zum Sicherstellen der Kompatibilität mit Microsoft R Client
...
### <a name="r-components-missing-from-cu3-setup"></a>R-Komponenten fehlen CU3-setup
...
### <a name="unable-to-install-python-components-in-offline-installations-of-sql-server-2017-ctp-20-or-later"></a>Kann nicht zum Installieren von Python-Komponenten in offline-Installationen von SQL Server 2017 CTP 2.0 oder höher
...
> *Der SQL Server Launchpad-Dienst konnte aufgrund des folgenden Fehlers nicht gestartet: der Dienst hat nicht auf die Start- oder Anforderung rechtzeitig reagiert.*
...
### <a name="remote-compute-contexts-are-blocked-by-a-firewall-in-sql-server-instances-that-are-running-on-azure-virtual-machines"></a>Remoterechenkontexte werden durch eine Firewall in SQL Server-Instanzen blockiert, die auf Azure Virtual Machines ausgeführt werden
...
Öffnen Sie dieses Problem zu umgehen, klicken Sie auf der Azure-VM **Windows-Firewall mit erweiterter Sicherheit**Option **Ausgangsregeln**, und deaktivieren Sie die folgende Regel: **Blockieren des Netzwerkzugriffs für lokale R-Benutzerkonten in SQL Server-Instanz MSSQLSERVER**. Sie können auch die Regel aktiviert lassen, aber ändern Sie die Eigenschaft für die Sicherheit auf **zulassen bei sicheren Verbindungen**.
...
### <a name="performance-limits-when-libraries-used-by-sql-server-are-called-from-other-tools"></a>Leistungseinschränkungen, wenn Bibliotheken, die von SQL Server verwendet, die von anderen Tools aufgerufen werden
...
> *TSQL-Modell: Fehler beim reverse engineering der Datenbank. Die Berechtigung wurde nicht erkannt und nicht importiert wurde.*
...
### <a name="external-script-execution-is-throttled-due-to-resource-governance-default-values"></a>Externen skriptausführung wird aufgrund der Standardwerte der Ressourcenkontrolle gedrosselt.
...
### <a name="access-denied-warning-when-executing-r-scripts-on-sql-server-in-a-non-default-location"></a>Warnung beim Ausführen von R-Skripts zu SQL Server auf einem nicht standardmäßigen Speicherort Zugriff verweigert
...
### <a name="serialization-error-between-old-and-new-versions-of-revoscaler"></a>Serialisierungsfehler zwischen alten und neuen Version von RevoScaleR
...
### <a name="real-time-scoring-does-not-correctly-handle-the-learningrate-parameter-in-tree-and-forest-models"></a>Echtzeitbewertung nicht ordnungsgemäß behandelt die _LearningRate_ Parameter in der Struktur und Gesamtstruktur-Modellen
...
### <a name="limits-on-size-of-serialized-models"></a>Grenzwerte für Größe der serialisierten Modelle
...
### <a name="avoid-clearing-workspaces-when-you-execute-r-code-in-a-includessnoversionincludesssnoversion-mdmd-compute-context"></a>Beim Ausführen von R-Code in das Löschen von Arbeitsbereichen vermeiden einer [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] compute Context verwenden
...
### <a name="limited-support-for-sql-data-types-in-spexecuteexternalscript"></a>Eingeschränkte Unterstützung für SQL-Datentypen in sp\_ausführen\_externe\_Skript
...
Roundtrips von Zeichenfolgen von [!INCLUDE[tsql](../includes/tsql-md.md)] in R und anschließend in [!INCLUDE[tsql](../includes/tsql-md.md)] erneut können zu Beschädigungen führen. Dies liegt an der verwendet wird, in R und in verschiedenen Codierungen [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)], sowie die verschiedenen Sortierungen und Sprachen, die in R unterstützt werden und [!INCLUDE[tsql](../includes/tsql-md.md)]. Zeichenfolgen mit einer Nicht-ASCII-Codierung werden möglicherweise nicht ordnungsgemäß verarbeitet.

Wenn Sie Zeichenfolgendaten an R senden, konvertieren Sie es nach Möglichkeit in eine ASCII-Darstellung.

Diese Einschränkung gilt für Daten, die auch zwischen SQL Server und Python zu übergeben. Multibyte-Zeichen als UTF-8 übergeben und als Unicode gespeichert werden.
...
### <a name="variable-scoping-error-when-you-use-the-transformfunc-parameter"></a>Fehler Bereichsdefinition, wenn Sie den Parameter TransformFunc Verwenden von Variablen
...
Parallelität für `rxExec` für mehrere Prozesse ist für eine künftige Version geplant.
...
### <a name="error-when-saving-serialized-model-to-sql-server"></a>Fehler beim Speichern serialisierte Modell in SQL Server
