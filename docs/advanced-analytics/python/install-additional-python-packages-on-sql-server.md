...
## <a name="list-installed-packages-using-conda"></a>Liste der installierten Pakete mit conda
...
Wenn Sie die Python-Befehlszeile verwenden, können Sie entweder **Pip** oder **Conda** Paket-Manager, mit der Python Anaconda-Umgebung hinzugefügt, die von SQL Server-Setup enthalten.

Vorausgesetzt, dass Sie den Ordner "Skripts" der PATH-Umgebungsvariable hinzugefügt, führen Sie diesen Befehl in eine Administratorbefehlszeile, die Pakete in Ihrer Umgebung Python aufzulisten. Andernfalls finden Sie unter [R abrufen und Python-Paketinformationen](../r/determine-which-packages-are-installed-on-sql-server.md#pip-conda) für Zeiger auf die Python-Tools in SQL Server ausführen.
