...
Die v3. 5-Version von DMA umfasst die folgenden Hinzufügungen:
- Beträchtliche Leistungssteigerungen für die Migration zu Azure SQL-Datenbank (Vergleichstests anzugeben, dass der Prozess viermal schneller als in früheren Versionen von DMA ist).
- Der Speicherbedarf ist weiter optimiert, um die Stabilität der Workflow bei der Migration zu verbessern.
- Die Fähigkeit zum Überspringen von Bewertungen während der Migrationen Schema und Daten, (falls Sie bereits die Bewertung ausgeführt und behandelt alle wichtigen Schemaobjekte vor der Migration).
- Ein Update auf ein Problem mit dem Tool stürzt ab, wenn ein ungültiger Netzwerkfreigabepfad für Sicherungsdateien bereitgestellt wird, beim Ausführen eines Upgrades von einer älteren Version von SQL Server auf eine höhere Version oder SQL Server auf Azure-VMs lokaler behandeln.
...
Linux und Windows verwenden unterschiedliche pfadformate. Migrieren zu SQL Server-2017 unter Linux folglich erfordert, dass der Benutzer auf dem Windows- und Linux-Versionen des Pfads zum Speicherort der physischen Datei angeben. Sie können beide Versionen des Pfads unterschiedlich, je nach Speicherort der physischen Datei bereitstellen.
...
Während die Migration von Active Directory (AD)-Anmeldungen von SQL Server-2017 unter Linux offiziell unterstützt wird, benötigt er zusätzliche Konfiguration ordnungsgemäß funktioniert. Finden Sie unter [Active Directory-Authentifizierung mit SQL Server on Linux](https://docs.microsoft.com/en-us/sql/linux/sql-server-linux-active-directory-authentication) für ausführliche Informationen zum Einrichten von Active Directory-Anmeldungen auf SQL Server-2017 unter Linux. Nach dem Ausführen der erforderlichen Konfigurations ist das Setup abgeschlossen ist und Sie können Active Directory-Anmeldungen wie gewohnt migrieren. Standard-SQL-Authentifizierung funktioniert erwartungsgemäß ohne dass eine zusätzliche Konfiguration.
