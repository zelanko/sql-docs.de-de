...
Der OLE DB-Treiber für SQL Server-Header- und-Bibliotheksdateien (msoledbsql.h und msoledbsql.lib) werden im installiert `%PROGRAMFILES%\Microsoft SQL Server\Client SDK\OLEDB\182\SDK`. Darüber hinaus die msoledbsql.msi die gleichen Dateien in installiert X64 `%PROGRAMFILES(x86)%\Microsoft SQL Server\Client SDK\OLEDB\182\SDK`.  
