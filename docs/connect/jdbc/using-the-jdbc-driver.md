...
ms.locfileid: "38015821"
---
# <a name="using-the-jdbc-driver"></a>Verwenden des JDBC-Treibers
[!INCLUDE[Driver_JDBC_Download](../../includes/driver_jdbc_download.md)]

  Dieser Abschnitt enthält Anleitungen, um in kurzer Zeit mit [!INCLUDE[ssNoVersion](../../includes/ssnoversion_md.md)] eine einfache Verbindung mit einer [!INCLUDE[jdbcNoVersion](../../includes/jdbcnoversion_md.md)]-Datenbank herzustellen. Vor dem Herstellen einer Verbindung mit einer [!INCLUDE[ssNoVersion](../../includes/ssnoversion_md.md)]-Datenbank müssen [!INCLUDE[ssNoVersion](../../includes/ssnoversion_md.md)] auf dem lokalen Computer oder einem Server sowie der JDBC-Treiber auf dem lokalen Computer installiert werden.  
  
## <a name="choosing-the-right-jar-file"></a>Auswählen der richtigen JAR-Datei  
 Der Microsoft JDBC-Treiber 6.4 für SQL Server bietet **Mssql-Jdbc-6.4.0.jre7.jar**, **Mssql-Jdbc-6.4.0.jre8.jar**, und **Mssql-Jdbc-6.4.0.jre9.jar** -Klassenbibliothek Dateien, die je nach Ihren bevorzugten Einstellungen für die Java Runtime Environment (JRE) verwendet werden.  
 
 Der Microsoft JDBC-Treiber 6.2 für SQL Server bietet **Mssql-Jdbc-6.2.1.jre7.jar**, und **Mssql-Jdbc-6.2.1.jre8.jar** Klassenbibliotheksdateien, abhängig von Ihrer bevorzugten Java-Laufzeit verwendet werden soll Umgebungseinstellungen (JRE).  
  
  Die Microsoft JDBC-Treiber 6.0 und 4.2 für SQL Server stellen die Klassenbibliotheksdateien **sqljdbc41.jar,** und **sqljdbc42.jar** zur Verfügung, die je nach Ihren bevorzugten JRE-Einstellungen verwendet werden sollen.  
  
 Der Microsoft JDBC-Treiber 4.1 für SQL Server stellt die Klassenbibliotheksdatei **sqljdbc41.jar** zur Verfügung, die je nach Ihren bevorzugten JRE-Einstellungen verwendet werden soll.  
...
## <a name="setting-the-classpath"></a>Festlegen des Klassenpfads  
 Der JDBC-Treiber ist kein Bestandteil des Java SDK. Wenn Sie ihn verwenden möchten, müssen Sie den Klassenpfad so festlegen, dass die Datei **sqljdbc.jar**, **sqljdbc4.jar**, **sqljdbc41.jar** oder **sqljdbc42.jar** eingeschlossen ist. Wenn der JDBC-Treiber 6.2, legen Sie den Klassenpfad aufnehmen der **Mssql-Jdbc-6.2.1.jre7.jar** oder **Mssql-Jdbc-6.2.1.jre8.jar**. Wenn der JDBC-Treiber 6.4, legen Sie den Klassenpfad aufnehmen der **Mssql-Jdbc-6.4.0.jre7.jar**, **Mssql-Jdbc-6.4.0.jre8.jar** oder **Mssql-Jdbc-6.4.0.jre9.jar**. Wenn die Angabe des Klassenpfads fehlt, gibt die Anwendung die allgemeine Ausnahme „Klasse nicht gefunden“ aus.  
  
### <a name="for-microsoft-jdbc-driver-64"></a>Für den Microsoft JDBC-Treiber 6.4
 ...
 Sie müssen sicherstellen, dass die CLASSPATH-Anweisung nur einen enthält [!INCLUDE[jdbcNoVersion](../../includes/jdbcnoversion_md.md)], z. B. entweder "Mssql-Jdbc-6.4.0.jre7.jar", "Mssql-Jdbc-6.4.0.jre8.jar" oder "Mssql-Jdbc-6.4.0.jre9.jar.   

### <a name="for-microsoft-jdbc-driver-62"></a>Für den Microsoft JDBC-Treiber 6.2
 Die **Mssql-Jdbc-6.2.1.jre7.jar** oder **Mssql-Jdbc-6.2.1.jre8.jar** Dateien werden am folgenden Speicherort installiert:  
 ...
 Sie müssen sicherstellen, dass die CLASSPATH-Anweisung nur einen enthält [!INCLUDE[jdbcNoVersion](../../includes/jdbcnoversion_md.md)], z. B. entweder "Mssql-Jdbc-6.2.1.jre7.jar" oder "Mssql-Jdbc-6.2.1.jre8.jar.  
...
### <a name="for-microsoft-jdbc-driver-41-42-and-60"></a>Für den Microsoft JDBC-Treiber 6.0, 4.2 und 4.1
 ...
 Sie müssen sicherstellen, dass die CLASSPATH-Anweisung nur einen [!INCLUDE[jdbcNoVersion](../../includes/jdbcnoversion_md.md)] enthält, beispielsweise sqljdbc4.jar, sqljdbc41.jar oder sqljdbc42.jar.  
...
```  
 ...
 In der JDBC API 4.0 wurde die DriverManager.getConnection Methode verbessert, um die JDBC-Treiber automatisch geladen. Daher müssen Anwendungen die Methode Class.forName bei Verwendung der Klassenbibliothek sqljdbc4.jar, sqljdbc41.jar oder sqljdbc42.jar nicht aufrufen, um den Treiber zu registrieren oder zu laden.  
