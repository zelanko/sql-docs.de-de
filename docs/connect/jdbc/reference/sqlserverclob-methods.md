---
title: SQLServerClob-Methoden | Microsoft-Dokumentation
ms.custom: ''
ms.date: 01/19/2017
ms.prod: sql
ms.prod_service: connectivity
ms.reviewer: ''
ms.technology: connectivity
ms.topic: conceptual
ms.assetid: 839f9c67-4cd4-4e07-a0ef-ba62591e9e5a
author: MightyPen
ms.author: genemi
ms.openlocfilehash: d4f1f7dbbd782f9ef1544c01d7fa7175b4416992
ms.sourcegitcommit: b2464064c0566590e486a3aafae6d67ce2645cef
ms.translationtype: MTE75
ms.contentlocale: de-DE
ms.lasthandoff: 07/15/2019
ms.locfileid: "67971749"
---
# <a name="sqlserverclob-methods"></a>SQLServerClob-Methoden
[!INCLUDE[Driver_JDBC_Download](../../../includes/driver_jdbc_download.md)]

  Informationen zu den Elementen der [SQLServerClob](../../../connect/jdbc/reference/sqlserverclob-class.md) -Klasse finden Sie unter [SQLServerClob](../../../connect/jdbc/reference/sqlserverclob-members.md)-Member.  
  
  
