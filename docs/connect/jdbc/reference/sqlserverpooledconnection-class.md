---
title: SQLServerPooledConnection-Klasse | Microsoft-Dokumentation
ms.custom: ''
ms.date: 01/19/2017
ms.prod: sql
ms.prod_service: connectivity
ms.reviewer: ''
ms.technology: connectivity
ms.topic: conceptual
ms.assetid: 4c87e9d3-904a-4b86-90cc-3684034c86ee
author: MightyPen
ms.author: genemi
manager: jroth
ms.openlocfilehash: 5cb094f135bb5e1bb25c3f7afb94517169035266
ms.sourcegitcommit: ad2e98972a0e739c0fd2038ef4a030265f0ee788
ms.translationtype: MTE75
ms.contentlocale: de-DE
ms.lasthandoff: 06/07/2019
ms.locfileid: "66791651"
---
# <a name="sqlserverpooledconnection-class"></a>SQLServerPooledConnection-Klasse
[!INCLUDE[Driver_JDBC_Download](../../../includes/driver_jdbc_download.md)]

  Stellt eine physische Datenbankverbindung in einem Verbindungspool dar.  
  
 **Paket:** com.microsoft.sqlserver.jdbc  
  
 **Erweitert:** java.lang.Object  
  
 **Implementiert:** javax.sql.PooledConnection  
  
## <a name="syntax"></a>Syntax  
  
```  
  
public final class SQLServerPoolingDataSource  
```  
  
## <a name="remarks"></a>Remarks  
 Die SQLServerPooledConnection-Klasse stellt Methoden für den Verbindungspool-Manager zum Verwalten des Verbindungspools bereit. Die Verbindungen werden in der Regel nicht direkt von Anwendungen instanziiert.  
  
## <a name="see-also"></a>Weitere Informationen  
 [SQLServerPooledConnection-Elemente](../../../connect/jdbc/reference/sqlserverpooledconnection-members.md)   
 [API-Referenz für den JDBC-Treiber](../../../connect/jdbc/reference/jdbc-driver-api-reference.md)  
  
  
