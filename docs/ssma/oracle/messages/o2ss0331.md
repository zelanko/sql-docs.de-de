...
In diesem Artikel wird beschrieben, warum SQL Server Migration Assistant (SSMA) für Oracle beim Umrechnen der- `FETCH ... INTO` Anweisung in einer-Prozedur, in der `SYS_REFCURSOR` als-Parameter zurückgegeben wird, einen Fehler liefert.

## <a name="background"></a>Hintergrund

In Oracle `SYS_REFCURSOR` wird verwendet, um Cursor von und an eine gespeicherte Prozedur zu übergeben.

Wenn SSMA auf eine Variable für `SYS_REFCURSOR` als Rückgabetyp der Prozedur trifft und dieselbe Variable mit der- `FETCH ... INTO` Anweisung verwendet, wird eine Fehlermeldung generiert.
