...
In diesem Artikel wird erläutert, warum SQL Server Migration Assistant (SSMA) für Oracle die PL/SQL-Blöcke `SELECT` nicht konvertiert, wenn `BULK COLLECT INTO` die- `DISTINCT` Anweisung mit der-Klausel und der-Klausel verwendet wird.
...
SSMA stellt einen Migrationspfad für Oracle-Sammlungen bereit. Wenn jedoch die `SELECT` -Anweisung zum Auffüllen der Auflistung eine `DISTINCT` -Klausel verwendet, generiert SSMA die Fehlermeldung.

## <a name="example"></a>Beispiel

Im folgenden Beispiel wird die- `SELECT` Anweisung mit `DISTINCT` der-Klausel und `BULK COLLECT INTO` -Klausel verwendet:
...
Um den obigen PL/SQL-Block zu konvertieren, müssen `DISTINCT` wir die `UNIQUE` Klausel mit der Klausel ( `DISTINCT`ein Synonym von) ersetzen, wie unten gezeigt:
