...
-   Speicheroptimierte Tabellen werden in Dateien beibehalten, die über einen oder mehrere Container verteilt sind. Jeder Container sollte in der Regel einer eigenen Spindel zugeordnet sein. Er wird sowohl für eine höhere Speicherkapazität als auch für eine verbesserte Leistung verwendet. Sie müssen sicherstellen, dass bei sequenziellen IOPS auf dem Speichermedium der dreifache Wert des Transaktionsprotokolldurchsatzes unterstützt wird.  
     ...
     Z. B. wenn Speicheroptimierte Tabellen Aktivitäten mit 500 MB/s im Transaktionsprotokoll generiert, muss der Speicher für Speicheroptimierte Tabellen 1,5 GB/s unterstützen. Die Notwendigkeit zur Unterstützung von einem 3 Mal transaktionsprotokolldurchsatzes ergibt sich daraus die Beobachtung, dass die Daten- / änderungsdateipaaren zuerst mit den anfänglichen Daten geschrieben werden und gelesen und erneut geschrieben werden, müssen als Teil eines Zusammenführungsvorgangs.  
...
-   Wenn eine gerade Anzahl an Spindeln vorliegt, erstellen Sie zweimal so viele Container, und ordnen Sie jeweils ein Paar derselben Spindel zu. Dies ist erforderlich, um die IOPS- und Speicherlast zu verteilen. Weitere Informationen finden Sie unter [speicheroptimierten Dateigruppe](the-memory-optimized-filegroup.md).  
