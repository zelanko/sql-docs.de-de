...
-   Das System toleriert keine Datenverluste.   
    Informationen zu möglichen Datenverlusten finden Sie im Abschnitt [Wann können Daten verloren gehen?](control-transaction-durability.md#bkmk_dataloss) .  
...
-   Die Transaktionsdauerhaftigkeit ist erst gewährleistet, nachdem das In-Memory-Transaktionsprotokoll auf den Datenträger geleert wurde. Das In-Memory-Transaktionsprotokoll wird in folgenden Fällen auf den Datenträger geleert:  
 ...
 Das Protokoll kann regelmäßig auf den Datenträger geleert werden. Allerdings [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] keine Garantie der Dauerhaftigkeit außer bei dauerhaften Transaktionen und Sp_flush_log.  
...
```  
 ...
 `ALLOWED`  
 Mit dieser Einstellung wird die Dauerhaftigkeit jeder Transaktion auf der Transaktionsebene bestimmt: DELAYED_DURABILITY = { *OFF* | ON }. Weitere Informationen finden Sie unter [Steuerung auf Atomic-Blockebene – systemintern kompilierte gespeicherte Prozeduren](control-transaction-durability.md#compiledproccontrol) und [Steuerung auf COMMIT-Ebene – Transact-SQL](control-transaction-durability.md#bkmk_t-sqlcontrol) .  
...
```  
 ...
 `OFF`  
 [Standard] Die Transaktion ist vollständig dauerhaft, es sei denn, die Datenbankoption DELAYED_DURABLITY = FORCED ist aktiviert, wodurch das COMMIT asynchron und daher verzögert dauerhaft ist. Weitere Informationen finden Sie unter [Database level control](control-transaction-durability.md#bkmk_dbcontrol) .  
  
 `ON`  
 Die Transaktion ist verzögert dauerhaft, es sei denn, die Datenbankoption DELAYED_DURABLITY = DISABLED ist aktiviert, wodurch das COMMIT synchron und daher vollständig dauerhaft ist.  Weitere Informationen finden Sie unter [Database level control](control-transaction-durability.md#bkmk_dbcontrol) .  
...
```  
 ...
 `OFF`  
 [Standard] Das COMMIT der Transaktion ist vollständig dauerhaft, es sei denn, die Datenbankoption DELAYED_DURABLITY = FORCED ist aktiviert, wodurch das COMMIT asynchron und daher verzögert dauerhaft ist. Weitere Informationen finden Sie unter [Database level control](control-transaction-durability.md#bkmk_dbcontrol) .  
  
 `ON`  
 Das COMMIT der Transaktion ist verzögert dauerhaft, es sei denn, die Datenbankoption DELAYED_DURABLITY = DISABLED ist aktiviert, wodurch das COMMIT synchron und daher vollständig dauerhaft ist. Weitere Informationen finden Sie unter [Database level control](control-transaction-durability.md#bkmk_dbcontrol) .  
