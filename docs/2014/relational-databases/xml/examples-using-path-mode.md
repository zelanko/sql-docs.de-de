...
```  
 ...
 Beachten Sie, dass das `MI` -Präfix auch in `WITH XMLNAMESPACES`definiert ist. Daher die `query()` Methode der `xml` angegebenen Typ definiert das Präfix nicht im abfrageprolog. Dies ist das Ergebnis:  
