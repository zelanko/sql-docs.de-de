 ...
 *event_property*  
 Eine Ereigniseigenschaft. Beispiele hierfür sind `PostTime`, `SPID` und `LoginName`. Suchen Sie jedes Ereignis in aufgeführten [WMI-Anbieter für Server Events-Ereignisklassen und Eigenschaften](wmi-provider-for-server-events-classes-and-properties.md) um zu bestimmen, welche Eigenschaften enthält. Beispielsweise enthält das DDL_DATABASE_LEVEL_EVENTS-Ereignis die Eigenschaften `DatabaseName` und `UserName`. Es erbt außerdem die Eigenschaften `SQLInstance`, `LoginName`, `PostTime` `SPID` und `ComputerName` von den jeweils übergeordneten Ereignissen.  
