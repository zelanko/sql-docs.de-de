...
-   Einen Satz von [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] -Cmdlets, die [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] -Befehle implementieren. Eines der Cmdlets ist **Invoke-Sqlcmd**. Dies dient zum Ausführen [!INCLUDE[ssDE](../../includes/ssde-md.md)] -abfrageskripts für die Ausführung mit der `sqlcmd` Hilfsprogramm.  
...
-   Die `sqlcmd` -Befehlszeilenprogramm können Sie interaktiv ausführen [!INCLUDE[ssDE](../../includes/ssde-md.md)] Abfragen und auch vorhandene [!INCLUDE[ssDE](../../includes/ssde-md.md)] Skriptdateien abzufragen.  
...
-   Verwenden der **-i *** Input_file* Parameter zum Ausführen der Datei mit den `sqlcmd` Hilfsprogramm.  
...
|Beschreibt, wie die `sqlcmd` -Hilfsprogramm ausführen [!INCLUDE[tsql](../../includes/tsql-md.md)] Skripts von der Befehlszeile aus, einschließlich der Möglichkeit zum interaktiven Entwickeln von Skripts.|[Themen zur Vorgehensweise für sqlcmd](../../database-engine/sqlcmd-how-to-topics.md)|  
