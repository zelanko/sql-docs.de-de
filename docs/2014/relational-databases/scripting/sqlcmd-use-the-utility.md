...
-   Die Server Option ( **-S**), mit der die Instanz [!INCLUDE[msCoName](../../includes/msconame-md.md)] von [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] identifiziert `sqlcmd` wird, mit der eine Verbindung hergestellt wird.  
...
### <a name="f-using-sqlcmd-in-a-windows-script-file"></a>F. Verwenden von "sqlcmd" in einer Windows-Skriptdatei  
 Ein `sqlcmd` Befehl`sqlcmd -i C:\InputFile.txt -o C:\OutputFile.txt,` wie kann in einer bat-Datei in Verbindung mit VBScript ausgeführt werden. In diesem Fall werden keine interaktiven Optionen verwendet. `sqlcmd` muss auf dem Computer installiert sein, auf dem die BAT-Datei ausgeführt wird.  
...
### <a name="g-using-sqlcmd-to-set-encryption-on-azure-sql-database"></a>G. Verwenden von sqlcmd zum Festlegen der Verschlüsselung für Azure SQL-Datenbank  
