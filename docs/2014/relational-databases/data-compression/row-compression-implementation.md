...
-   Es verwendet Speicherformat variabler Länge für numerische Datentypen (z. B. `integer`, `decimal`, und `float`) und die Typen, die auf numerischen Werten basieren (z. B. `datetime` und `money`).  
...
|`float`|Benutzerkontensteuerung|Geringwertige Bytes mit Nullen werden nicht gespeichert. `float` -Komprimierung eignet sich hauptsächlich für nicht gebrochene Werte in Mantissen.|  
|`real`|Benutzerkontensteuerung|Geringwertige Bytes mit Nullen werden nicht gespeichert. `real` -Komprimierung eignet sich hauptsächlich für nicht gebrochene Werte in Mantissen.|  
