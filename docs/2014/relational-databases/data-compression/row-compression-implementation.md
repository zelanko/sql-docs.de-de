...
-   Es verwendet ein Speicherformat mit variabler Länge für numerische Typen (z. B. `integer`, `decimal`, und `float`) und die Typen, die auf numerischen Werten basieren (z. B. `datetime` und `money`).  
