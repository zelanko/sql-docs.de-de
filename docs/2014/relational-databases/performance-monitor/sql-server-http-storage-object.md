...
# <a name="sql-server-http_storage_object"></a>SQL Server, HTTP_STORAGE_OBJECT
  Das Leistungs Objekt **SQLServer: HTTP_STORAGE_OBJECT** besteht aus Leistungsindikatoren, die Azure Storage Konto überwachen. Wenn Sie [SQL Server Datendateien in Azure](../databases/sql-server-data-files-in-microsoft-azure.md) verwenden, können Sie Datenbankdateien in Azure Storage blobspeicher speichern. Dieses Leistungs Objekt behandelt jedes Azure Storage Konto als ein anderes Laufwerk.  
