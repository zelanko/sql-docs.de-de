...
|DatabaseID|`int`|ID der Datenbank, in der das Timeout auftrat. [!INCLUDE[ssSqlProfiler](../../includes/sssqlprofiler-md.md)] Zeigt den Namen der Datenbank, wenn die `ServerName` -Datenspalte in der Ablaufverfolgung aufgezeichnet wird und der Server verfügbar ist. Der Wert für eine Datenbank kann mithilfe der DB_ID-Funktion ermittelt werden.|3|Benutzerkontensteuerung|  
