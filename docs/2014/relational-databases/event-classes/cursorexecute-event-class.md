...
# <a name="cursorexecute-event-class"></a>CursorExecute-Ereignisklasse
  Die **CursorExecute** -Ereignisklasse beschreibt Cursor Ausführungs Ereignisse, die in API-Cursorn (Application Programming Interface) auftreten. Cursor Ausführungs Ereignisse treten auf [!INCLUDE[msCoName](../../includes/msconame-md.md)] [!INCLUDE[ssDE](../../includes/ssde-md.md)] , wenn erstellt und einen Cursor aus dem Ausführungsplan auffüllt, der von einem Cursor Vorbereitungs Ereignis erstellt wurde.  
...
|**Bewältigen**|**int**|Ganze Zahl, die von ODBC, OLE DB oder DB-Library zum Koordinieren der Ausführung mit dem Server verwendet wird.|33|Ja|  
