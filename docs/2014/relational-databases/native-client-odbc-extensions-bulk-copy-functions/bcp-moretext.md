...
## <a name="remarks"></a>Hinweise  
 Diese Funktion kann verwendet werden, zusammen mit [Bcp_bind](bcp-bind.md) und [Bcp_sendrow](bcp-sendrow.md) langen Daten variabler Länge Werte mit SQL Server in einer Reihe von kleineren Blöcken kopiert. **Bcp_moretext** kann mit Spalten mit den folgenden SQL Server-Datentypen verwendet werden: `text`, `ntext`, `image`, `varchar(max)`, `nvarchar(max)`, `varbinary(max)`, den benutzerdefinierten Typ (UDT) und XML. **bcp_moretext** unterstützt keine Datenkonvertierungen. Die angegebenen Daten müssen mit dem Datentyp der Zielspalte übereinstimmen.  
  
 Wenn **Bcp_bind** mit einem ungleich NULL aufgerufen wird *pData* -Parameter für Datentypen, die von unterstützt werden **Bcp_moretext**, `bcp_sendrow` sendet den gesamten Datenwert unabhängig der Länge. IF, allerdings **Bcp_bind** enthält eine NULL *pData* -Parameter für unterstützte Datentypen **Bcp_moretext** können verwendet werden, um Daten unmittelbar nach einer erfolgreichen Rückgabe von Kopieren`bcp_sendrow` , der angibt, dass alle gebundenen Spalten mit vorhandenen Daten verarbeitet wurden.  
 ...
 Aufruf eines **Bcp_bind** oder [Bcp_collen](bcp-collen.md) legt die gesamte Länge aller datenausschnitte in der SQL Server-Spalte kopiert werden sollen. Beim Senden von SQL Server mehr als im Aufruf angegebenen Bytes **Bcp_bind** oder `bcp_collen` wird ein Fehler generiert. Dieser Fehler tritt z. B. in einer Anwendung, die verwendet `bcp_collen` , legen Sie die Länge der verfügbaren Daten für eine SQL Server `text` Spalte 4500, dann aufgerufen, **Bcp_moretext** fünfmal beim bei jedem Aufruf wurde, dass die Länge des Datenpuffers 1000 Byte lang.  
 ...
 Eine Anwendung ruft normalerweise `bcp_sendrow` und **Bcp_moretext** in Schleifen, um eine Anzahl von Zeilen mit Daten zu senden. Hier wird ein Überblick über die Vorgehensweise für eine Tabelle mit zwei `text` Spalten:  
...
## <a name="example"></a>Beispiel  
 Dieses Beispiel zeigt, wie **Bcp_moretext** mit **Bcp_bind** und `bcp_sendrow`:  
