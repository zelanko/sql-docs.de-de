...
> [!NOTE]  
>  Wenn Sie einen wiederherstellungstask angeben, indem [!INCLUDE[ssManStudioFull](../../includes/ssmanstudiofull-md.md)], können Sie generieren, das entsprechende [!INCLUDE[tsql](../../includes/tsql-md.md)] [wiederherstellen](/sql/t-sql/statements/restore-statements-transact-sql) Skript durch Klicken auf **Skript** und anschließend ein Ziel für das Skript auswählen.  
...
## <a name="permissions"></a>Berechtigungen  
 ...
 Erfordert eine verschlüsselte Sicherung wiederherstellen `VIEW DEFINITION` Berechtigungen für das Zertifikat oder den asymmetrischen Schlüssel verwendet, um während der Sicherung zu verschlüsseln.  
...
## <a name="restoring-from-an-encrypted-backup"></a>Wiederherstellen aus einer verschlüsselten Sicherung  
 Für die Wiederherstellung muss das Zertifikat oder der asymmetrische Schlüssel, mit dem die Datenbank ursprünglich erstellt wurde, auf der Instanz verfügbar sein, auf der die Wiederherstellung erfolgen soll. Das Konto, das Ausführen der Wiederherstellung müssen `VIEW DEFINITIONS` für das Zertifikat oder asymmetrischen Schlüssel. Zertifikate, die zum Verschlüsseln von Sicherungen verwendet wurden, sollten nicht erneuert oder aktualisiert werden.  
  
## <a name="restoring-from-windows-azure-storage"></a>Wiederherstellen aus dem Windows Azure-Speicher  
 Wenn Sie eine Sicherung wiederherstellen, die im Windows Azure-Speicher gespeichert ist, weist die Wiederherstellungsbenutzeroberfläche eine neue Sicherungsmedienoption auf. **URL** auf die **Sicherungsmedien auswählen** Dialogfeld. Beim Klicken auf **hinzufügen**, wird die **mit Windows Azure verbinden** Dialogfeld, das Ihnen ermöglicht, geben Sie die SQL-Anmeldeinformationen für das Speicherkonto zu authentifizieren.  Nach Herstellen der Verbindung mit dem Speicherkonto werden die Sicherungsdateien angezeigt, der **eine Sicherungsdatei in Windows Azure suchen** Dialogfeld, in dem Sie die Datei für die Wiederherstellung verwendet auswählen können.  
