...
-   Ein oder mehrere bestimmte Werte sind für das Schlüsselfeld oder die Schlüsselfelder im Updategram angegeben.  
 ...
 Wenn entweder der Fall ist, verwendet das Updategram die Schlüsselspalten, die im angegebenen der `sql:key-fields` auf die Elemente in der  **\<vor >** und  **\<nach >** Blöcke.  
 ...
 Die Einträge, die im angegebenen der  **\<vor >** und  **\<nach >** Blöcke müssen nicht in derselben Reihenfolge befinden. Die `updg:id` -Attribut erzwingt die Zuordnung zwischen den Elementen, die im angegebenen der  **\<vor >** und  **\<nach >** blockiert.  
  
 Wenn Sie angeben, dass ein Element in der  **\<vor >** Block und nur ein entsprechendes Element in der  **\<nach >** mit blockiert `updg:id` ist nicht erforderlich. Es wird jedoch empfohlen, `updg:id` trotzdem anzugeben, um Mehrdeutigkeit zu vermeiden.  
