...
-   Ein oder mehrere bestimmte Werte sind für das Schlüsselfeld oder die Schlüsselfelder im Updategram angegeben.  
 ...
 Wenn entweder der Fall ist, verwendet das Updategram die im angegebenen Schlüsselspalten der `sql:key-fields` auf die Elemente in der  **\<vor >** und  **\<nach >** Blöcke.  
 ...
 Die Einträge, die im angegebenen die  **\<vor >** und  **\<nach >** Blöcke müssen nicht in der gleichen Reihenfolge angegeben werden. Die `updg:id` -Attribut erzwingt die Zuordnung zwischen den Elementen, die im angegebenen die  **\<vor >** und  **\<nach >** Blöcke.  
  
 Wenn Sie angeben, dass ein Element in der  **\<vor >** Block und nur ein entsprechendes Element im der  **\<nach >** blockieren, verwenden `updg:id` ist nicht erforderlich. Es wird jedoch empfohlen, `updg:id` trotzdem anzugeben, um Mehrdeutigkeit zu vermeiden.  
...
```  
 ...
 Beachten Sie, dass wie die `updg:id` -Attribut ordnet die erste Instanz von der \<HumanResources.Shift >-Element in der  **\<vor dem >** Block mit der zweiten Instanz von der \< HumanResources.Shift >-Elements in der  **\<nach >** Block.  
...
3.  Erstellen und verwenden Sie das SQLXML 4.0-Testskript (Sqlxml4test.vbs), um das Updategram auszuführen.  
 ...
 Weitere Beispiele für Updategrams, die Zuordnungsschemas verwenden, finden Sie unter [ein Mapping-Schema mit Anmerkungen angeben, in einem Updategram &#40;SQLXML 4.0&#41;](specifying-an-annotated-mapping-schema-in-an-updategram-sqlxml-4-0.md).  
