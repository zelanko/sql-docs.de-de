---
title: MDSCHEMA_PROPERTIES-Rowset | Microsoft-Dokumentation
ms.custom: ''
ms.date: 03/06/2017
...
|`PROPERTY_TYPE`|`DBTYPE_I2`||Eine Bitmap, die den Typ der Eigenschaft angibt:<br /><br /> -   `MDPROP_MEMBER` (`1`) gibt eine Eigenschaft eines Elements. Diese Eigenschaft kann in der DIMENSION PROPERTIES-Klausel der SELECT-Anweisung verwendet werden.<br />-   `MDPROP_CELL` (`2`) gibt eine Eigenschaft einer Zelle. Diese Eigenschaft kann in der CELL PROPERTIES-Klausel verwendet werden, die am Ende der SELECT-Anweisung vorkommt.<br />-   `MDPROP_SYSTEM` (`4`) gibt eine interne Eigenschaft.<br />-   `MDPROP_BLOB` (`8`) gibt eine Eigenschaft, die ein binary large Object (Blob) enthält.|  
...
|`PROPERTY_ORIGIN`|`DBTYPE_UI2`||Gibt den Typ der Hierarchie an, für die die Eigenschaft gilt:<br /><br /> -   `MD_USER_DEFINED` (`1`) gibt an, dass die Eigenschaft in einer benutzerdefinierten Hierarchie<br />-   `MD_SYSTEM_ENABLED` (`2`) gibt an, dass die Eigenschaft für eine Attributhierarchie<br />-   `MD_SYSTEM_DISABLED` (`4`) gibt an, dass die Eigenschaft für eine Attributhierarchie, die nicht aktiviert ist.|  
