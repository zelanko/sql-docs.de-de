---
title: Herstellen einer Verbindung mit Quelldaten (Datamining-Client für Excel) | Microsoft Docs
ms.custom: ''
ms.date: 12/29/2017
...
1.  Klicken Sie auf die **Verbindung** Schaltfläche auf der **analysieren** oder **Data Mining** Menüband.  
...
3.  In der **neue Analysis Services-Verbindung** Dialogfeld Geben Sie den Namen des Servers.  
  
4.  Geben Sie die Authentifizierungsmethode an.  
  
5.  Wählen Sie eine Datenbank aus der **Katalogname** Dropdown-Liste. Wenn für die Instanz keine Datenbank vorhanden ist, wählen Sie **(Standard)**.  
  
6.  Geben Sie einen Anzeigenamen für die Verbindung ein.  
  
7.  Klicken Sie auf **Testverbindung** um sicherzustellen, dass der Server und Datenbank verfügbar sind.  
...
### <a name="connections-in-the-visio-add-in"></a>Verbindungen im Visio-Add-In  
 ...
 Wenn Sie eine Verbindung mit zuvor verwendet haben eine [!INCLUDE[ssASnoversion](../includes/ssasnoversion-md.md)] -Datenquellen in Excel, werden diese Verbindungen als mögliche Datenquellen zur Auswahl aufgeführt.  
...
3.  In der **wählen Sie eine Datenquelle** (Dialogfeld), wählen Sie eine Datenquelle aus der Liste aus, oder klicken Sie auf **neu**.  
  
4.  Bei Auswahl des **neu**, verwenden Sie das Verfahren, die weiter oben beschrieben wird, geben Sie einen Server und der Name des Katalogs oder Herstellen einer Verbindung über einen Webdienst.  
...
##  <a name="bkmk_change"></a> Ändern von Verbindungen  
 ...
 Im Data Mining-Client für Excel, können Sie auch die Verbindungszeichenfolge und den Status für die aktuelle Verbindung überprüfen, indem Sie auf **Ablaufverfolgung** und dann auf **aktuelle Verbindung**.  
  
#### <a name="use-a-different-server-connection"></a>Verwenden einer anderen Serververbindung  
  
1.  Klicken Sie auf **Verbindung**.  
  
2.  In der **Analysis Services-Verbindungen** Bereich eine Verbindung aus der **andere Verbindungen** aus, und klicken Sie auf **als aktuell**.  
...
2.  Wenn Sie die Verbindungszeichenfolge ändern müssen, um das Abfragetimeout zu erhöhen oder der Instanz von [!INCLUDE[ssASnoversion](../includes/ssasnoversion-md.md)] weitere Parameter hinzuzufügen, besteht eine Möglichkeit darin, die DMC-Datei mit der gespeicherten Verbindungszeichenfolge zu bearbeiten.  
     ...
     \<Laufwerk: > \Users\\< MeinBenutzername\>\AppData\Local\Microsoft\Data Mining-Add-in  
  
##  <a name="bkmk_extconnections"></a> Herstellen einer Verbindung mit externen Datenquellen  
 Während die Tools in der **analysieren** arbeiten ausschließlich mit Daten in Excel, die Tools im Menüband die **Data Mining** Menüband können Sie die direkte Verbindung mit externen Datenquellen, die als Eingaben verwendet werden sollen, für das Modell oder für Sampling.  
