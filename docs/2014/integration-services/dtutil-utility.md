...
>  Zahlreiche dtutil-Vorgänge können auch visuell über eine Instanz von [!INCLUDE[ssManStudioFull](../includes/ssmanstudiofull-md.md)] in [!INCLUDE[ssISnoversion](../includes/ssisnoversion-md.md)] ausgeführt werden. Weitere Informationen finden Sie unter [Paketverwaltung &#40;SSIS-Dienst&#41;](service/package-management-ssis-service.md).  
 ...
 Die Optionen können in beliebiger Reihenfolge eingegeben werden. Der senkrechte Strich ("|") dient als `OR`-Operator und wird verwendet, um mögliche Werte anzuzeigen. Sie müssen eine der Optionen, die als Trennzeichen sind verwenden die `OR` Pipe.  
...
-   Öffnen Sie ein Eingabeaufforderungsfenster, wechseln Sie zum Verzeichnis mit der 64-Bit-Version des Hilfsprogramms *(\<Laufwerk >*: \Programme\Microsoft SQL Server\120\DTS\Binn), und führen Sie das Hilfsprogramm aus diesem Verzeichnis.  
  
-   Führen Sie das Hilfsprogramm an der Eingabeaufforderung, indem Sie den vollständigen Pfad eingeben (*\<Laufwerk >*: \Programme\Microsoft SQL Server\120\DTS\Binn) auf die 64-Bit-Version des Hilfsprogramms.  
  
-   Ändern Sie die Reihenfolge der Pfade in der PATH-Umgebungsvariablen dauerhaft, indem platzieren den 64-Bit-Pfad (*\<Laufwerk >*: \Programme\Microsoft SQL Server\120\DTS\Binn) vor dem 32-Bit-Pfad ( *\<Laufwerk >*: \ Programm Dateien (x86) \Microsoft SQL Server\120\DTS\Binn) in der Variablen.  
...
> [!NOTE]  
>  Um ein Paket von einem benannten Server zu löschen, enthalten die `SOURCESERVER` Option und die zugehörigen Argumente. Die Angabe eines Servers ist nur möglich, wenn Sie die Option *SQL* verwenden.  
...
```  
 ...
 Verwenden Sie **CertMgr**, um Informationen zu Ihrem Zertifikat zu finden. Sie können den Hashcode im Hilfsprogramm **CertMgr** anzeigen, indem Sie das Zertifikat auswählen und anschließend auf **Anzeigen** klicken, um die Eigenschaften anzuzeigen. Die Registerkarte **Details** enthält zusätzliche Informationen zu dem Zertifikat. Die `Thumbprint` Eigenschaft dient als Hashwert, Leerzeichen entfernt wurden.  
