...
|Eigenschaft|Description|  
|--------------|-----------------|  
|`DelayValidation`|Zeigt an, ob die Paketüberprüfung bis zur Ausführung des Pakets verschoben wird. Der Standardwert für diese Eigenschaft ist `False`.|  
...
|`PackagePriorityClass`|Die Win32-Threadprioritätsklasse des Paketthreads. Die Werte sind `Default`, `AboveNormal`, `Normal`, `BelowNormal`, `Idle`. Der Standardwert dieser Eigenschaft ist `Default`. Weitere Informationen finden Sie unter <xref:Microsoft.SqlServer.Dts.Runtime.DTSPriorityClass>.|  
...
|`PackageType`|Der Pakettyp. Die Werte sind `Default`, `DTSDesigner`, `DTSDesigner100`, `DTSWizard`, `SQLDBMaint`, und `SQLReplication`. Der Standardwert dieser Eigenschaft ist `Default`. Weitere Informationen finden Sie unter <xref:Microsoft.SqlServer.Dts.Runtime.DTSPackageType>.|  
