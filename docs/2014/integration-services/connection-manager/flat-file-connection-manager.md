...
## <a name="configuration-of-the-flat-file-connection-manager"></a>Konfiguration des Verbindungs-Managers für Flatfiles  
 Wenn Sie einen Flatfile-Verbindungs-Manager einem Paket hinzufügen [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] [!INCLUDE[ssISnoversion](../../includes/ssisnoversion-md.md)] erstellt eine Verbindung-Manager, der in ein Flatfile-Verbindung zur Laufzeit aufgelöst wird, legt die Eigenschaften der Flatfile-Verbindung und fügt den Flatfile-Verbindungs-Manager, um die `Connections` -Auflistung des Pakets.  
  
 Die `ConnectionManagerType` des Verbindungs-Managers ist-Eigenschaftensatz auf `FLATFILE`.  
 ...
 In einigen Fällen wird die Paketleistung verbessert, wenn Sie diese Funktion deaktivieren. Sie können diese Funktion deaktivieren, indem Sie die Flatfile Verbindungs-Manager-Eigenschaft **AlwaysCheckForRowDelimiters**in `False`.  
