...
# <a name="upgrade-integration-services"></a>Upgrade von Integration Services
 ...
 Durch diese parallele Installation werden mehrere Versionen des Hilfsprogramms dtexec installiert. Um sicherzustellen, dass Sie die richtige Version des Hilfsprogramms ausführen, führen Sie an der Eingabeaufforderung das Hilfsprogramm aus, indem Sie den vollständigen Pfad ( \<drive> : \Programme\Microsoft SQL Server \\<Version \> \dt\binn) eingeben. Weitere Informationen zu dtexec finden Sie unter [dtexec (Hilfsprogramm)](../packages/dtexec-utility.md).  
