...
7.  Geben Sie in der Main-Prozedur Code zum Protokollieren der Fehler durch Aufrufen der `Dts.Log` -Methode, oder um ein Ereignis auszulösen, durch den Aufruf einer der Methoden von der `Dts.Events` Schnittstelle. Informieren Sie das Paket über den Fehler, indem Sie `Dts.TaskResult = Dts.Results.Failure`zurückgeben.  
