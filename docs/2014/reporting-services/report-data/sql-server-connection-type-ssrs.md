...
-   Wenn Sie die Option für mehrwertige Parameter auswählen, müssen Sie manuell ändern, auf die Abfrage zu testen, ob Werte Teil einer Reihe mit dem `IN` Operator, z. B. `WHERE EmployeeID IN (@EmpID)`.  
