...
1.  Konfigurieren Sie das Konto für die unbeaufsichtigte Ausführung. Konfigurieren dieses Konto ist erforderlich für Verbindungen, die mit `IDbConnection`. Der Berichtsserver nimmt die Identität des Kontos beim Herstellen der Verbindung an.  
...
|Anmeldeinformationen|Verbindungen|  
|-----------------|-----------------|  
|Integrierte Sicherheit|Sofern vom Datenprovider unterstützt, können Sie integrierte Sicherheit von Windows bei benutzerdefinierten datenverarbeitungserweiterungen, mit denen `IDbConnectionExtension`.<br /><br /> Wenn Sie die Verbindungszeichenfolge zu definieren, werden Sie Argumente einschließen, die die integrierte Sicherheit angeben (z. B. eine Verbindung mit einer [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] -Datenquelle umfassen möglicherweise `Integrated Security=SSPI` in der Verbindungszeichenfolge).|  
|Windows-Authentifizierung|Sofern vom Datenprovider unterstützt, können Sie ein Windows-Domänenbenutzerkonto bei benutzerdefinierten datenverarbeitungserweiterungen, mit denen `IDbConnectionExtension`.<br /><br /> Der Berichtsserver nimmt vor dem Aufruf der Datenverarbeitungserweiterung die Identität des Benutzerkontos an. Wenn Sie die Verbindungszeichenfolge zu definieren, werden Sie Argumente einschließen, die die integrierte Sicherheit angeben (z. B. eine Verbindung mit einer [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] -Datenquelle umfassen möglicherweise `Integrated Security=SSPI` in der Verbindungszeichenfolge).|  
