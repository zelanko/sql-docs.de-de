...
-   Verwenden Sie NTLM. NTLM funktioniert im Allgemeinen in Fällen, in denen die Kerberos-Authentifizierung fehlschlägt. Zur Verwendung von NTLM entfernen `RSWindowsNegotiate` aus der Datei RSReportServer.config, und stellen Sie sicher, dass nur `RSWindowsNTLM` angegeben ist. Wenn Sie sich für diese Vorgehensweise entscheiden, können Sie auch dann weiterhin ein Domänenbenutzerkonto für den Berichtsserverdienst verwenden, wenn Sie keinen SPN dafür definiert haben.  
...
-   Ändern Sie die Konfigurationsdatei `rsreportserver.config` indem Sie folgende Einstellungen:  
