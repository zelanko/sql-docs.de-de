...
## <a name="return"></a>Rückgabewert  
 Gibt ein räumliches Objekt zurück. Dies ist je nach Ausdruckstyp vom Typ `SqlGeometry` oder `SqlGeography`. Weitere Informationen zu `SqlGeometry` und `SqlGeography` räumliche Datentypen finden Sie unter [Übersicht über räumliche Datentypen](../../relational-databases/spatial/spatial-data-types-overview.md).  
