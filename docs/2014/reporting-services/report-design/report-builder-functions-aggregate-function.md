 ...
 *Bereich*  
 (`String`) Der Namen eines Datasets, Gruppe oder eines Datenbereichs mit dem Bericht Berichtselementen, auf die die Aggregatfunktion anzuwenden. *Bereich* muss eine Zeichenfolgenkonstante sein und darf kein Ausdruck sein. Wenn *scope* nicht angegeben ist, wird der aktuelle Bereich verwendet.  
  
## <a name="return-type"></a>Rückgabetyp  
 Wird durch den Datenanbieter bestimmt. Gibt `Nothing` Wenn der Datenanbieter diese Funktion nicht unterstützt oder Daten nicht verfügbar sind.  
...
## <a name="remarks"></a>Hinweise  
 ...
 [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] die Verwendung von erkennt die `Aggregate` -Funktion in Ausdrücken Ihres Berichts, um zu ermitteln, ob Serveraggregate in Detailzeilen anzeigen. Wenn Sie aufnehmen `Aggregate` in einem Ausdruck in einem Datenbereich können Serveraggregate nur in Ergebniszeilen für Gruppen oder gesamtergebniszeilen, nicht in Detailzeilen erscheinen. Wenn Sie Serveraggregate in Detailzeilen anzeigen möchten, verwenden Sie die `Aggregate`-Funktion nicht.  
