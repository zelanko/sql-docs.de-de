...
|**Ja**|Gibt an, ob JavaScript im gerenderten Bericht unterstützt wird. Standardwert: `true`.|  
...
|**Parameter**|Gibt an, ob der Parameterbereich der Symbolleiste angezeigt oder ausgeblendet werden soll. Wenn Sie diesen Parameter auf den Wert `true` festlegen, wird der Parameterbereich der Symbolleiste angezeigt. Der Standardwert dieses Parameters lautet `true`.|  
...
|**`Section`**|Die Seitenzahl des zu rendernden Berichts. Der Wert `0` gibt an, dass alle Abschnitte des Berichts gerendert werden. Standardwert: `1`.|  
