...
##  <a name="URLreservation"></a> Registrierung und Reservierung für URLs  
 Eine URL-Reservierung definiert die URLs, über die auf eine [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] -Anwendung zugegriffen werden kann. [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] eine oder mehrere URLs für den Berichtsserver-Webdienst und Berichts-Manager in HTTP-reserviert. SYS, und registriert diese, wenn der Dienst gestartet wird. Die URLs für den Berichts-Generator und die Berichte basieren auf der URL-Reservierung für den Berichtsserver-Webdienst. Sie können Berichts-Generator oder Berichte über den Webdienst öffnen, indem Sie Parameter an die URL anfügen. Reservierungen und Registrierungen werden von HTTP.SYS bereitgestellt. Weitere Informationen finden Sie unter [Namespacereservierungen, Registrierung und Routing](http://go.microsoft.com/fwlink/?LinkId=92653) auf MSDN.  
...
|http://123.45.67.0:80/reportserver|http://123.45.67.0/reportserver|Diese URL-Reservierung gibt eine IP-Adresse an und ist viel restriktiver als die Platzhalter-URL-Reservierung. Nur mit URLs, die eine IP-Adresse enthalten, kann eine Verbindung mit dem Berichtsserver hergestellt werden. Diese URL-Reservierung kann eine Anforderung an einen Berichtsserver unter http:// erhält\<Computername > / Reportserver oder http://localhost/reportserver fehlschlagen.|  
...
##  <a name="URLPermissionsAccounts"></a> Authentifizierung und Dienstidentität für Reporting Services-URLs  
 ...
 Anonymer Zugriff ist deaktiviert, da die Standardsicherheit ist `RSWindowsNegotiate`. Berichtsserver-URLs verwenden Netzwerkcomputernamen für den Intranetzugriff. Wenn Sie [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] für Internetverbindungen konfigurieren möchten, müssen Sie andere Einstellungen verwenden. Weitere Informationen finden Sie in der [-Onlinedokumentation unter](../security/authentication-with-the-report-server.md) Authentifizierung beim Berichtsserver [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] .  
...
-   Für Bereitstellungen auf einem Server, auf dem ein integrierter Berichtsserver und SharePoint-technologieinstanz auf demselben Computer installiert sind, können keine http://localhost/reportserver. Wenn http://localhost wird verwendet, um die SharePoint-Webanwendung zuzugreifen, müssen Sie mithilfe einer nicht standardmäßigen Website oder eine eindeutige portzuweisung einen Berichtsserver zugreifen. Außerdem kann der localhost-Zugriff auf einen Berichtsserver nicht für Knoten in der Bereitstellung auf Remotecomputern aufgelöst werden, wenn der Berichtsserver in eine SharePoint-Farm integriert ist.  
