---
title: Untersuchung des Forecasting-Modells (mittleres Datamining-Lernprogramm) | Microsoft Docs
ms.custom: ''
ms.date: 03/06/2017
...
# <a name="exploring-the-forecasting-model-intermediate-data-mining-tutorial"></a>Prüfen des Planungserstellungsmodells (Data Mining-Lernprogramm für Fortgeschrittene)
  Nun, dass Sie das forecasting-Miningmodell erstellt haben, können Sie die Ergebnisse untersuchen, mit der **Miningmodell-Viewer** des Data Mining-Designers. Die [!INCLUDE[msCoName](../includes/msconame-md.md)] Time Series-Viewer enthält zwei Registerkarten: **Diagramme** und **Modell**.  
...
##  <a name="bkmk_Charts"></a> Registerkarte "Diagramme"  
 Die **Diagramme** auf der Registerkarte die [!INCLUDE[msCoName](../includes/msconame-md.md)] Time Series-Viewer grafisch wird veranschaulicht, wie Sie der Reihe, einschließlich der Vergangenheitsdaten und Vorhersagen. Jede Zeile im Zeitreihendiagramm stellt eine eindeutige Kombination von Produkt, Bereich und vorhersagbarem Attribut dar.  
...
1.  Klicken Sie auf die **Diagramme** auf der Registerkarte die **Miningmodell-Viewer** Registerkarte, wenn es nicht angezeigt wird.  
...
3.  Deaktivieren Sie die Kontrollkästchen rechts vom Diagramm, um die Zeilen für alle auf dem Betrag basierenden Reihen vorübergehend auszublenden.  
 ...
 Den Trendlinien können Sie entnehmen, dass der Gesamtumsatz für alle Regionen zunimmt, wobei alle 12 Monate im Dezember Spitzenwerte verzeichnet werden. Aus dem Diagramm können Sie auch sehen, dass die Daten für das Fahrrad T1000 deutlich später beginnen als die Daten für die anderen Produktreihen startet. Grund hierfür ist die Tatsache, dass es sich um ein neueres Produkt handelt. Da diese Reihe eine deutlich geringere Datengrundlage hat, sind die Vorhersagen möglicherweise nicht so genau.  
...
1.  Versuchen Sie es ändern des Werts für **Vorhersageschritte** schrittweise Erhöhung des von **5** auf **10**, und klicken Sie dann zurück an **6**.  
...
3.  Beachten Sie die Skala der X-Achse. Die Änderungen an Vergangenheits- und vorhergesagten Daten werden immer als Prozentsatz ausgedrückt; die Istwerte werden jedoch automatisch so angepasst, dass alle Werte in das Diagramm passen. Daher dürfen Sie beim Vergleichen von Modellen nicht nur die grafische Darstellung berücksichtigen. Um die genaue abzurufen, oder die prozentuale Steigerung und dem Wert für Vorhersagen verwenden, lassen Sie den Mauszeiger über die gepunktete Linie oder durchgezogene Linien, oder klicken Sie auf die Linien zum Anzeigen der Werte in der **Mininglegende**.  
 ...
 [Zurück nach oben](#bkmk_Charts)  
  
##  <a name="bkmk_Model"></a> Registerkarte "Modell"  
 Die **Modell** auf der Registerkarte die [!INCLUDE[msCoName](../includes/msconame-md.md)] Time Series-Viewer im Data Mining-Designer können Sie der forecasting-Modells in Form von ein Strukturdiagramm anzeigen.  
...
-   Wenn ein Strukturdiagramm für eine Zeitreihe mehrere Verzweigungen hat, bedeutet dies, dass die erkannte Zeitreihe zu komplex für eine Darstellung als einzelne Gleichung ist. Stattdessen Strukturdiagramm mehrere Verzweigungen, wobei jede Verzweigung, die mit den Bedingungen, die die Struktur verursacht beschriftet enthalten möglicherweise *teilen*. Wenn sich die Struktur teilt, stellt jede Verzweigung ein Zeitsegment dar, innerhalb dessen der Trend als einzelne Gleichung beschrieben werden kann.  
  
     Beispielsweise, wenn Sie das Diagramm ansehen und einen abrupten Sprung in Verkaufsvolumen irgendwann im September beginnt und über einen Feiertag, Sie können zum Wechseln der **Modell** Ansicht das genaue Datum der trendänderung. Die Verzweigungen in der Struktur, die "vor September" und "nach September" darstellen, enthalten unterschiedliche Formeln: eine Formel, die den Verkaufstrend bis zur Teilung mathematisch beschreibt, und eine andere Formel, die den Verkaufstrend von September bis zum Feiertag am Jahresende beschreibt.  
...
1.  In der **Struktur** auf in der Liste der **Modell** wählen auf der Registerkarte die **T1000 Europe: Amount** Reihe.  
     ...
     Für eine **alle** Knoten, die QuickInfo, die angezeigt wird enthält Informationen, z. B. die Anzahl der Fälle in der gesamten Reihe und zeitreihenformeln abgeleitet Analyse der Daten.  
...
3.  Wählen Sie jetzt eine andere Zeitreihe zur Anzeige aus. Mit der **Struktur** auf in der Liste der **Modell** wählen auf der Registerkarte der **M200 North America: Amount** Reihe.  
     ...
     Für jeden untergeordneten Knoten, die Beschreibung in der **Mininglegende** enthält auch die Anzahl der Fälle in jeder Verzweigung der Struktur.  
  
 Die folgende Liste beschreibt einige zusätzliche Funktionen des Struktur-Viewers:  
  
-   Sie können ändern, dass die Variable, die im Diagramm, mithilfe dargestellt wird der **Hintergrund** Steuerelement. Standardmäßig enthalten dunklere Knoten mehr Fälle, da der Wert der **Hintergrund** festgelegt ist, um **Auffüllung**. Um festzustellen, wie viele Fälle gibt es nur einen anderen Knoten sind halten Sie die Maus auf einen Knoten, und zeigen Sie die QuickInfo, die angezeigt wird, oder klicken Sie auf den Knoten und zeigen Sie die Anzahl in der **Knotenlegende** Fenster.  
...
##  <a name="bkmk_Content"></a> (Optional) Generic Content Tree Viewer  
 Zusätzlich zu den benutzerdefinierten Viewer für Zeitreihen stellt [!INCLUDE[ssASnoversion](../includes/ssasnoversion-md.md)] bietet die **MicrosoftGeneric Content Tree Viewer** für die Verwendung mit allen Datamining-Modelle. Dieser Viewer bietet einige Vorteile:  
...
-   **Generic Content Tree Viewer**: ermöglicht Ihnen das Durchsuchen und alle Datenreihen, die verwendet wurden, gleichzeitig in das Modell anzeigen und wenn Sie ein gemischtes erstellt haben, zu modellieren, die ARIMA- und ARTXP-Strukturen im gleichen Diagramm angezeigt.  
...
4.  In der **Knotenbeschriftung** Bereich, suchen Sie den ersten Knoten, die über untergeordnete Knoten verfügt.  
     ...
     Wenn ein Reihe Knoten untergeordnete Elemente verfügt, der Strukturansicht angezeigten auf die **Modell** der Microsoft Time Series-Viewer auf der Registerkarte müssen zudem eine Verzweigungsstruktur.  
