...
|sys.dm_os_memory_cache_counters|Die folgenden Spalten wurden umbenannt:<br /><br /> Es ist jetzt Single_pages_kb: <br />                          pages_kb<br /><br /> multi_pages_kb<br />                           ist jetzt: Pages_in_use_kb|  
|sys.dm_os_memory_cache_entries|Die Spalte Pages_allocated_count wurde umbenannt Pages_kb.|  
|sys.dm_os_memory_clerks|Die Spalte Multi_pages_kb wurde entfernt.<br /><br /> Die Spalte Single_pages_kb wurde umbenannt Pages_kb.|  
|sys.dm_os_memory_nodes|Die folgenden Spalten wurden umbenannt:<br /><br /> Es ist jetzt Single_pages_kb: <br />                            pages_kb<br /><br /> Es ist jetzt Multi_pages_kb: <br />                            foreign_committed_kb|  
|sys.dm_os_memory_objects|Die folgenden Spalten wurden umbenannt.<br /><br /> Es ist jetzt Pages_allocated_count:<br />                            pages_in_bytes<br /><br /> Max_pages_allocated_count ist jetzt: Max_pages_in_bytes|  
|sys.dm_os_sys_info|Die folgenden Spalten wurden umbenannt:<br /><br /> Es ist jetzt Physical_memory_in_bytes: <br />                            physical_memory_kb<br /><br /> Es ist jetzt Bpool_commit_target: <br />                            committed_target_kb<br /><br /> Es ist jetzt Bpool_visible: <br />                            visible_target_kb<br /><br /> Es ist jetzt Virtual_memory_in_bytes: <br />                            virtual_memory_kb<br /><br /> Es ist jetzt Bpool_commited:<br />                            committed_kb|  
...
|CLR-Routinen|Durch Identitätswechsel innerhalb der benutzerdefinierten CLR-Funktionen, benutzerdefinierten Aggregate oder benutzerdefinierten Typen (User-Defined Types, UDTs) können kann nach dem Upgrade auf [!INCLUDE[ssKatmai](../includes/sskatmai-md.md)] bei der Anwendung der Fehler 6522 auftreten. Die folgenden Szenarios können in [!INCLUDE[ssVersion2005](../includes/ssversion2005-md.md)] erfolgreich ausgeführt werden, nicht jedoch in [!INCLUDE[ssKatmai](../includes/sskatmai-md.md)]. Für jedes Szenario werden Auflösungen bereitgestellt.<br /><br /> Eine CLR User-defined Function, benutzerdefinierte Aggregate oder UDT-Methode, die ein Identitätswechsel verwendet weist einen Parameter vom Typ `nvarchar(max)`, `varchar(max)`, `varbinary(max)`, `ntext`, `text`, `image`, oder einen umfangreichen UDT und verfügt nicht über die  **DataAccessKind.Read** Attribut in der Methode. Um dieses Problem zu beheben, fügen die **DataAccessKind.Read** -Attribut der Methode, kompilieren Sie die Assembly und die Routine und die Assembly erneut bereitstellen.<br /><br /> Eine CLR-Tabellenwertfunktion-Funktion, die verfügt über eine **Init** Methode, die Identitätswechsel ausführt. Zum Beheben dieses Problems fügen die **DataAccessKind.Read** -Attribut der Methode, kompilieren Sie die Assembly und die Routine und die Assembly erneut bereitstellen.<br /><br /> Eine CLR-Tabellenwertfunktion-Funktion, die verfügt über eine **FillRow** Methode, die Identitätswechsel ausführt. Um dieses Problem zu beheben, entfernen Sie Identitätswechsel von der **FillRow** Methode. Nicht auf externe Ressourcen mithilfe der **FillRow** Methode. Stattdessen Zugriff auf externe Ressourcen aus der **Init** Methode.|  
...
|Funktion|Description|  
|-------------|-----------------|  
|Showplan (XML-Schema)|Ein neues **SeekPredicateNew** Element wird hinzugefügt, um die Showplan XML-Schema und die einschließende XSD-Sequenz (**SqlPredicatesType**) wird konvertiert in eine  **\<xsd: Choice >** Element. Statt eine oder mehrere **SeekPredicate** Elemente, die eine oder mehrere **SeekPredicateNew** Elemente können jetzt in der Showplan XML angezeigt werden. Die beiden Elemente schließen sich gegenseitig aus. **SeekPredicate** bleibt in der Showplan XML-Schema für Abwärtskompatibilität Kompatibilität; allerdings Abfragepläne in erstellten [!INCLUDE[ssKatmai](../includes/sskatmai-md.md)] darf die **SeekPredicateNew** Element. Anwendungen, die voraussichtlich nur abgerufen werden. die **SeekPredicate** untergeordnetes Element aus dem Knoten ShowPlanXML/BatchSequence/Batch/Statements/StmtSimple/QueryPlan/RelOp/IndexScan/SeekPredicates kann fehlschlagen, wenn die  **SeekPredicate** Element ist nicht vorhanden. Schreiben Sie die Anwendung entweder erwartet die **SeekPredicate** oder **SeekPredicateNew** Element in diesem Knoten. Weitere Informationen finden Sie unter .|  
|Showplan (XML-Schema)|Ein neues **IndexKind** -Attribut hinzugefügt, die **ObjectType** komplexen Typ in der Showplan XML-Schema. Anwendungen, die eine strenge Validierung von [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)]-Plänen anhand des [!INCLUDE[ssVersion2005](../includes/ssversion2005-md.md)]-Schemas durchführen, generieren einen Fehler.|  
  
### <a name="transact-sql"></a>Transact-SQL  
  
|Funktion|Description|  
|-------------|-----------------|  
|ALTER_AUTHORIZATION_DATABASE DDL-Ereignis|In [!INCLUDE[ssVersion2005](../includes/ssversion2005-md.md)], wenn das DDL-Ereignis ALTER_AUTHORIZATION_DATABASE ausgelöst wird, den Wert "object" werden zurückgegeben, in der **ObjectType** für dieses Ereignis, wenn der Entitätstyp des sicherungsfähigen Elements in die Definition der XML-EventData-Element Datendefinitionssprache (DDL) Vorgang ist ein Objekt. In [!INCLUDE[ssKatmai](../includes/sskatmai-md.md)] wird der tatsächliche Typ (z. B. "table" oder "function") zurückgegeben.|  
...
|sp_helpuser|Die folgenden Spaltennamen, die im Resultset der gespeicherten Prozedur Sp_helpuser zurückgegeben werden, wurden geändert:<br /><br /> Es ist jetzt Gruppenname:<br />                            RoleName<br /><br /> Es ist jetzt Gruppenname:<br />                            Role_name<br /><br /> Es ist jetzt Group_id:<br />                            Role_id<br /><br /> Es ist jetzt Users_in_group:<br />                            Users_in_role|  
...
|XQuery und Xpath-Ausdrücke|In [!INCLUDE[ssVersion2005](../includes/ssversion2005-md.md)], die Schritte in einem XQuery- oder XPath-Ausdruck, der mit einem Doppelpunkt (': ') sind zulässig. Die folgende Anweisung enthält zum Beispiel einen Namenstest (`CTR02)`) innerhalb des Pfadausdrucks, der mit einem Doppelpunkt beginnt.<br /><br /> `SELECT FileContext.query('for n$ in //CTR return <C>{data )(n$/:CTR02)} </C>) AS Files FROM dbo.MyTable;`<br /><br /> In [!INCLUDE[ssKatmai](../includes/sskatmai-md.md)] ist diese Verwendung nicht zulässig, da sie nicht den XML-Standards entspricht. Der Fehler 9341 wird zurückgegeben. Entfernen Sie den führenden Doppelpunkt, oder geben Sie ein Präfix für den Namenstest an, z. B. (n$/p1:CTR02) oder (n$/CTR02).|  
