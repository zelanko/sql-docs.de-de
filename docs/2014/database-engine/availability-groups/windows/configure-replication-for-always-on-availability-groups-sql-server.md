...
1.  Konfigurieren Sie die Remoteverteilung. Wenn gespeicherte Prozeduren zur Konfiguration des Verlegers verwendet werden, führen Sie `sp_adddistributor` aus. Geben Sie den gleichen Wert für *@password* an, der verwendet wurde, als `sp_adddistrbutor` auf dem Verteiler ausgeführt wurde, um die Verteilung einzurichten.  
...
```  
 ...
 Die gespeicherte Prozedur `sp_validate_replica_hosts_as_publishers` sollte von einer Anmeldung mit ausreichender Autorisierung bei jedem Verfügbarkeitsgruppenreplikathost ausgeführt werden, um Informationen zur Verfügbarkeitsgruppe abzufragen. Im Gegensatz zu `sp_validate_redirected_publisher`werden die Anmelde Informationen des Aufrufers verwendet, und die in msdb. dbo. MSdistpublishers beibehaltene Anmeldung wird nicht zum Herstellen einer Verbindung mit den Verfügbarkeits Gruppen Replikaten verwendet.  
