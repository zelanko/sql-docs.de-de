...
2.  Verwenden der `Import-Module` Cmdlet zum Importieren des Sqlps-Moduls. Geben Sie die `DisableNameChecking` Parameter an, wenn Sie die Warnung zu unterdrücken möchten `Encode-Sqlname` und `Decode-Sqlname`.  
