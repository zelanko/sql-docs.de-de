---
title: Ändern von Änderungen an der Volltextsuche | Microsoft Docs
ms.custom: ''
ms.date: 03/06/2017
...
|*top_n_by_rank* Parameter (der CONTAINSTABLE- und [FREETEXTTABLE-Anweisungen)](/sql/relational-databases/system-functions/freetexttable-transact-sql) [!INCLUDE[tsql](../includes/tsql-md.md)]|*top_n_by_rank* Abfragen, die 0 als Parameter verwenden.|Erzeugt einen Fehler, da – wie in der Fehlermeldung angegeben – ein Wert größer als 0 (null) verwendet werden muss.|Wird erfolgreich ausgeführt und gibt 0 (null) Zeilen zurück.|  
...
|**Itemcount**|Tabelle enthält keine Dokumente oder Typspalten.|Zusätzlich zu indizierten Dokumenten werden Dokumente, die NULL sind oder Nulltypen aufweisen, im **ItemCount-Wert** gezählt.|Nur indizierte Dokumente werden im **ItemCount-Wert** gezählt.|  
|Katalog **ItemCount**|BLOB-Spalte mit einer NULL-Erweiterung.|Sie wird in ItemCount des Katalogs **gezählt.**|Sie wird in **ItemCount** of catalog nicht gezählt.|  
|**UniqueKeyCount**|Abfragen einer eindeutigen Anzahl von Schlüsseln von einem Katalog, z. B. zwei Tabellen (table1 und table2) mit jeweils drei Wörtern (word1, word2 und word3).|**UniqueKeyCount** = 9. Die folgende Tabelle fasst zusammen, wie dieser Wert erreicht wird:<br /><br /> table1 = 3<br /><br /> EOF für Volltextindex von table1 = 1<br /><br /> table2 = 3<br /><br /> EOF für Volltextindex von table2 = 1<br /><br /> Volltextkatalog = 1|Für jede Tabelle ist **UniqueKeyCount** die Anzahl der unterschiedlichen Schlüsselwörter + 1 (0xFF).  Dies behandelt NICHT die gleichen Wörter in > 1 Dokument als neuer eindeutiger Schlüssel.<br /><br /> Für einen Katalog ist **UniqueKeyCount** die Summe von **UniqueKeyCount** für jede der Tabellen unter dem Katalog. Identische Wörter aus verschiedenen Tabellen werden als eindeutige Schlüssel behandelt. In diesem Fall entspricht die Anzahl der eindeutigen Schlüssel 8.|  
|**Option auf Rangserverebene vorberechnen**|Leistungsoptimierung von FREETEXTTABLE-Abfragen.|Wenn die Option auf 1 festgelegt ist, verwenden FREETEXTTABLE-Abfragen, die mit *top_n_by_rank* angegebenen, vorberechnete Rangdaten, die in den Volltextkatalogen gespeichert sind.|Wird nicht unterstützt.|  
...
|Gebietsschemaspezifische Stoppwörter|Abfragen ingebietsspezifischen Varianten einer übergeordneten Sprache, z. B. Belgisches Französisch und Kanadisches Französisch.|Abfragen in gebietsschemaspezifischen Varianten werden von den Komponenten (Worttrennzeichen, Stammmer und Stoppwörter) ihrer übergeordneten Sprache verarbeitet. Beispielsweise wird Französisch (Belgien) mit den Komponenten von Französisch (Frankreich) analysiert.|Stoppwörter müssen jedem Gebietsschemabezeichner (LCID) explizit hinzugefügt werden. Beispielsweise müssen Sie eine LCID für Belgien, Kanada und Frankreich angeben.|  
...
|[DATABASEPROPERTYEX](/sql/t-sql/functions/databasepropertyex-transact-sql)|Verwenden der **IsFullTextEnabled-Eigenschaft.**|Die **Einstellung IsFullTextEnabled** gibt an, ob die Volltextsuche in einer bestimmten Datenbank aktiviert ist.|Der Wert dieser Spalte hat keine Auswirkungen. In Benutzerdatenbanken ist die Volltextsuche standardmäßig aktiviert.|  
  
## <a name="see-also"></a>Weitere Informationen  
 [Verhaltensänderungen bei der Volltextsuche](../relational-databases/search/full-text-search.md)   
