---
title: Fehlerhafte Änderungen an der Volltextsuche | Microsoft Docs
ms.custom: ''
ms.date: 03/06/2017
...
|*Top_n_by_rank* Parameter (von der CONTAINSTABLE und [FREETEXTTABLE](/sql/relational-databases/system-functions/freetexttable-transact-sql) [!INCLUDE[tsql](../includes/tsql-md.md)] Anweisungen)|*Top_n_by_rank* Abfragen, die mit dem Wert 0 als Parameter.|Erzeugt einen Fehler, da – wie in der Fehlermeldung angegeben – ein Wert größer als 0 (null) verwendet werden muss.|Wird erfolgreich ausgeführt und gibt 0 (null) Zeilen zurück.|  
...
|**ItemCount**|Tabelle enthält keine Dokumente oder Typspalten.|Neben indizierten Dokumenten Dokumente, die null sind oder deren Typ NULL, im gezählt werden, die **ItemCount** Wert.|Nur indizierte Dokumente im gezählt werden, die **ItemCount** Wert.|  
|Katalog **ItemCount**|BLOB-Spalte mit einer NULL-Erweiterung.|Wert entspricht **ItemCount** des Katalogs|Es wird nicht gezählt **ItemCount** des Katalogs.|  
|**UniqueKeyCount**|Abfragen einer eindeutigen Anzahl von Schlüsseln von einem Katalog, z. B. zwei Tabellen (table1 und table2) mit jeweils drei Wörtern (word1, word2 und word3).|**UniqueKeyCount** = 9. In der folgenden Tabelle werden die Entstehung dieses Werts zusammengefasst:<br /><br /> table1 = 3<br /><br /> EOF für Volltextindex von table1 = 1<br /><br /> table2 = 3<br /><br /> EOF für Volltextindex von table2 = 1<br /><br /> Volltextkatalog = 1|Für jede Tabelle **UniqueKeyCount** ist die Anzahl unterschiedlicher Schlüsselwörter + 1 (0xFF).  Dieselben Wörter in > 1.DOC werden nicht als neuer eindeutiger Schlüssel behandelt.<br /><br /> Für einen Katalog **UniqueKeyCount** ist die Summe der **UniqueKeyCount** der einzelnen Tabellen im Katalog. Identische Wörter aus verschiedenen Tabellen werden als eindeutige Schlüssel behandelt. In diesem Fall entspricht die Anzahl der eindeutigen Schlüssel 8.|  
|**vorausberechnen** auf Serverebene-Option|Leistungsoptimierung von FREETEXTTABLE-Abfragen.|Wenn die Option auf 1 festgelegt ist, angegebene FREETEXTTABLE-Abfragen mit *Top_n_by_rank* verwenden Sie vorausberechnete rangdaten, die in den Volltextkatalogen gespeichert.|Wird nicht unterstützt.|  
