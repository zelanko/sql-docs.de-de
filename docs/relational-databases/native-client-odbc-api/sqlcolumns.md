 ...
 **SQLColumns** kann auf einem statischen Servercursor ausgeführt werden. Der Versuch, **SQLColumns** auf einem aktualisierbaren Cursor (dynamisch oder Keyset) auszuführen, gibt SQL_SUCCESS_WITH_INFO zurück, der angibt, dass der Cursortyp geändert wurde.  
 ...
 Für ODBC 2. *x-Anwendungen,* die keine Platzhalter in *TableName*verwenden, gibt **SQLColumns** Informationen zu allen Tabellen zurück, deren Namen mit *TableName* übereinstimmen und die im Besitz des aktuellen Benutzers sind. Wenn der aktuelle Benutzer keine Tabelle besitzt, deren Name mit dem *TableName-Parameter* übereinstimmt, gibt **SQLColumns** Informationen zu allen Tabellen zurück, die anderen Benutzern gehören, bei denen der Tabellenname mit dem *TableName-Parameter* übereinstimmt. Für ODBC 2. *x-Anwendungen,* die Platzhalter verwenden, gibt **SQLColumns** alle Tabellen zurück, deren Namen mit *TableName*übereinstimmen. Für ODBC 3. *x-Anwendungen* **SQLColumns** gibt alle Tabellen zurück, deren Namen *TableName* unabhängig vom Besitzer oder ob Platzhalter verwendet werden.  
...
|SS_UDT_ASSEMBLY_TYPE_NAME|Der qualifizierte Name der UDT-Assembly.|  
 ...
 Bei UDTs wird die vorhandene TYPE_NAME Spalte verwendet, um den Namen der UDT anzugeben. Daher sollte keine zusätzliche Spalte für sie dem Resultset von **SQLColumns** oder [SQLProcedureColumns](../../relational-databases/native-client-odbc-api/sqlprocedurecolumns.md)hinzugefügt werden. Der DATA_TYPE für eine UDT-Spalte oder einen UDT-Parameter ist SQL_SS_UDT.  
 ...
 Wenn ein Client [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] eine Verbindung zu SQLColumns herstellt und diese aufruft, werden mithilfe von NULL- oder Platzhalterwerten für den Katalogeingabeparameter keine Informationen aus anderen Katalogen zurückgegeben. Stattdessen werden nur Informationen über den aktuellen Katalog zurückgegeben. Der Client kann zunächst SQLTables aufrufen, um zu bestimmen, in welchem Katalog sich die gewünschte Tabelle befindet. Der Client kann dann diesen Katalogwert für den Katalogeingabeparameter in seinem Aufruf an SQLColumns verwenden, um Informationen zu den Spalten in dieser Tabelle abzurufen.  
  
## <a name="sqlcolumns-and-table-valued-parameters"></a>SQLColumns und Tabellenwertparameter  
 Das von SQLColumns zurückgegebene Resultset hängt von der Einstellung SQL_SOPT_SS_NAME_SCOPE ab. Weitere Informationen finden Sie unter [SQLSetStmtAttr](../../relational-databases/native-client-odbc-api/sqlsetstmtattr.md). Die folgenden Spalten wurden für Tabellenwertparameter hinzugefügt:  
...
## <a name="sqlcolumns-support-for-sparse-columns"></a>SQLColumns-Unterstützung für Spalten mit geringer Dichte  
 Dem [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Resultset für SQLColumns wurden zwei spezifische Spalten hinzugefügt:  
...
|SS_IS_COLUMN_SET|**Smallint**|Wenn es sich bei der Spalte um die **column_set** Spalte handelt, ist dies SQL_TRUE. andernfalls SQL_FALSE.|  
 ...
 In Übereinstimmung mit der ODBC-Spezifikation werden SS_IS_SPARSE und SS_IS_COLUMN_SET vor [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] allen treiberspezifischen Spalten angezeigt, die Versionen vor versionen vor hinzugefügt [!INCLUDE[ssKatmai](../../includes/sskatmai-md.md)]wurden, und nach allen Spalten, die von ODBC selbst vorgeschrieben wurden.  
  
 Das von SQLColumns zurückgegebene Resultset hängt von der Einstellung SQL_SOPT_SS_NAME_SCOPE ab. Weitere Informationen finden Sie unter [SQLSetStmtAttr](../../relational-databases/native-client-odbc-api/sqlsetstmtattr.md).  
  
 Weitere Informationen zu spärlichen Spalten in ODBC finden Sie unter [Sparse Columns Support &#40;ODBC&#41;](../../relational-databases/native-client/odbc/sparse-columns-support-odbc.md).  
