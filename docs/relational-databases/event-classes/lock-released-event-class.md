...
|DatabaseID|**int**|Die ID der Datenbank, in der die Sperre freigegeben wurde. [!INCLUDE[ssSqlProfiler](../../includes/sssqlprofiler-md.md)] zeigt den Namen der Datenbank an, wenn die ServerName-Datenspalte in der Ablaufverfolgung aufgezeichnet wird und der Server verfügbar ist. Der Wert für eine Datenbank kann mithilfe der DB_ID-Funktion ermittelt werden.|3|Benutzerkontensteuerung|  
...
|ObjectID|**int**|Vom System zugewiesene ID des freigegebenen Objekts (sofern verfüg- und anwendbar).|22|Benutzerkontensteuerung|  
