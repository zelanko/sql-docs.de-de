...
|-----------------|---------------|-----------------|  
|\<Verfolgen Sie alle Spalten >|\<Variiert nach >|Die Ablaufverfolgungsdaten aus der Tabelle snapshots.trace_data in der Verwaltungs-Data Warehouse-Datenbank.<br /><br /> Mithilfe der folgenden Abfrage kann eine Liste der Spalten für die angegebene Ablaufverfolgung abgerufen werden:<br /><br /> `SELECT * FROM sys.trace_columns`<br /><br /> **Hinweis**: Die Werte in der Spalte "Name" in der trace_columns-Systemsicht entsprechen die Spalten, die von der fn_trace_gettable-Funktion zurückgegeben werden. Der einzige Unterschied ist, dass die Spalte GroupID von der Funktion nicht zurückgegeben wird.|  
