---
description: Sicherheitsarchitektur für die Websynchronisierung
...
|Authentifizierungstyp|Stelle, an der die Authentifizierung angegeben wird|  
|----------------------------|-------------------------------------------|  
|Windows-Authentifizierung|[!INCLUDE[tsql](../../../includes/tsql-md.md)]: die Parameter `@ftp_login` und `@ftp_password` von [sp_addmergepublication](../../../relational-databases/system-stored-procedures/sp-addmergepublication-transact-sql.md).<br /><br /> RMO: <xref:Microsoft.SqlServer.Replication.Publication.FtpLogin%2A> und <xref:Microsoft.SqlServer.Replication.Publication.FtpPassword%2A>.|  
