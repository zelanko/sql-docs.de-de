...
Wenn der Benutzer über `VIEW SERVER STATE` Berechtigung auf dem Server verfügt, werden dem Benutzer alle ausgeführten Sitzungen in der Instanz von [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]angezeigt. Andernfalls wird dem Benutzer nur die aktuelle Sitzung angezeigt. `VIEW SERVER STATE` kann nicht in [!INCLUDE[ssSDS_md](../../includes/sssds-md.md)] erteilt werden, sodass `sys.dm_exec_requests` immer auf die aktuelle Verbindung beschränkt ist.
...
Im folgenden Beispiel wird **sys. dm_exec_requests** abgefragt, um den interessanten Batch zu suchen und seine `transaction_id` aus der Ausgabe zu kopieren.
...
Verwenden Sie dann die kopierten `transaction_id` mit der Systemfunktion **sys. dm_tran_locks**, um Sperrinformationen zu finden.  
