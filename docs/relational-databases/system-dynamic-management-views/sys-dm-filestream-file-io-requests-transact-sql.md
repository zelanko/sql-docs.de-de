...
|**client_thread_id**|**varbinary(8)**|Zeigt die Thread-ID der Client Anwendung an, von der die Anforderung stammt.<br /><br /> ** \* Warnung \* \* ** Dies ist nur dann von Bedeutung, wenn die Client Anwendung auf dem gleichen Computer wie SQL Server ausgeführt wird. Wird die Clientanwendung remote ausgeführt, zeigt die **client_thread_id** die Thread-ID eines Systemprozesses an, der für den Remoteclient ausgeführt wird.<br /><br /> Lässt NULL-Werte zu.|  
