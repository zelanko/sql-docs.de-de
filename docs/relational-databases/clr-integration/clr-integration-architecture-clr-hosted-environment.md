---
title: CLR gehostete Umgebung | Microsoft Docs
description: In diesem Artikel wird beschrieben, wie CLR und SQL Server integriert werden, um Systemressourcen einheitlich zu verwalten, und wie DIE SICHERHEIT von CAS und SQL Server integriert ist.
ms.custom: ''
ms.date: 03/17/2017
ms.prod: sql
ms.reviewer: ''
...
###### <a name="host-protection-attributes-hpas"></a>Hostschutzattribute (HPAs)  
...
###### <a name="scalability-common-threading-scheduling-and-synchronization"></a>Skalierbarkeit: Allgemeines Threading, Planung und Synchronisierung  
 ...
 Es gibt einige Situationen, in denen lang andauernder verwalteter Code automatisch nachlässt, und einige Situationen, in denen dies nicht der Fall ist. In den folgenden Situationen liefert lang andauernder verwalteter Code automatisch:
 
 - Wenn der Code das SQL OS aufruft (z. B. um Daten abzufragen)
 - Wenn genügend Arbeitsspeicher zum Auslösen der Garbage Collection reserviert ist
 - Wenn der Code durch Aufrufen von OS-Funktionen in den Präventivmodus wechselt

 Code, der keines der oben genannten Probleme ausführt, z. B. enge Schleifen, die nur Berechnungen enthalten, liefert nicht automatisch den Planer, was zu langen Wartezeiten auf andere Workloads im System führen kann. In diesen Situationen ist es Sache des Entwicklers, explizit nachzugeben, indem er die System.Thread.Sleep()-Funktion von .NET Framework aufruft oder explizit den Präemtive-Modus mit System.Thread.BeginThreadAffinity() in allen Abschnitten von Code eingibt, die voraussichtlich lange ausgeführt werden. Die folgenden Codebeispiele zeigen, wie Sie mit jeder dieser Methoden manuell nachgeben.
...
###### <a name="reliability-application-domains-and-unrecoverable-exceptions"></a>Zuverlässigkeit: Anwendungsdomänen und nicht behebbare Ausnahmen  
 ...
 Beim Hosten in [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] werden solche Threadabbrüche folgendermaßen behandelt: Die CLR erkennt jeden Freigabezustand in der Anwendungsdomäne, in der der Threadabbruch auftritt. Die CLR erkennt dies, indem sie überprüft, ob Synchronisierungsobjekte vorliegen. Wenn es einen Freigabezustand in der Anwendungsdomäne gibt, dann wird die Anwendungsdomäne selbst entladen. Die Entladung aus der Anwendungsdomäne beendet Datenbanktransaktionen, die gerade in dieser Anwendungsdomäne ausgeführt werden. Da das Vorhandensein eines Freigabezustands die Auswirkungen dieser schwerwiegenden Ausnahmen auf Benutzersitzungen außer der Sitzung, die die Ausnahme ausgelöst hat, vergrößern kann, haben [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] und die CLR Schritte unternommen, um die Wahrscheinlichkeit eines Freigabezustands zu verringern. Weitere Informationen hierzu finden Sie in der .NET Framework- Dokumentation.  
...
### <a name="programming-model-restrictions"></a>Einschränkungen des Programmiermodells  
 ...
 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]Außerdem werden Aufrufe von .NET Framework-APIs, die mit den Hostschutzattributen **SharedState**, **Synchronization**und **ExternalProcessMgmt** versehen sind, nicht zudiesem. Dies verhindert, dass Assemblys des Typs SAFE und EXTERNAL_ACCESS APIs aufrufen, die die Freigabe des Zustands aktivieren, Synchronisierungen durchführen und die Integrität des [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]-Prozesses beeinträchtigen. Weitere Informationen finden Sie unter [CLR Integration Programming Model Restrictions](../../relational-databases/clr-integration/database-objects/clr-integration-programming-model-restrictions.md).  
