...
> [!NOTE]  
>  Bei Verwendung mit großen Datenmengen verbraucht die @**query_no_truncate** -Option zusätzliche Ressourcen und kann die Serverleistung verlangsamen.  
  
`[ @query_result_no_padding ] @query_result_no_padding`Der Typ ist "Bit". Die Standardeinstellung ist 0. Wenn Sie auf 1 festlegen, werden die Abfrageergebnisse nicht aufgefüllt, und möglicherweise wird die Dateigröße reduziert. Wenn Sie auf @query_result_no_padding 1 festlegen und den @query_result_width -Parameter festlegen, über @query_result_no_padding schreibt der-Parameter @query_result_width den-Parameter.  
  
 In diesem Fall tritt kein Fehler auf.  
 
  >[!NOTE]
  > Der folgende Fehler kann auftreten, wenn @query_result_no_padding auf 1 festgelegt und ein Parameter für bereitgestellt wird.@query_no_truncate:
  > <br> Meldung 22050, Ebene 16, Status 1, Zeile 0: Die Abfrage konnte nicht ausgeführt werden, @query_result_no_append da @query_no_truncate die Optionen und sich gegenseitig ausschließen. 
  
 Wenn Sie @query_result_no_padding auf 1 festlegen und den @query_no_truncate -Parameter festlegen, wird ein Fehler ausgelöst.  
