...
`[ @job_name = ] 'job_name'` den Namen des Auftrags, dem der Schritt hinzugefügt werden soll. *job_name* ist vom **Datentyp vom Datentyp sysname**und hat den Standardwert NULL.  
...
`[ @step_name = ] 'step_name'` den Namen des Schritts. *step_name* ist vom **Datentyp vom Datentyp sysname**und hat keinen Standardwert.  
...
`[ @command = ] 'command'` die Befehle, die vom **SQLServerAgent** -Dienst über das *Subsystem*ausgeführt werden. der *Befehl* ist vom Datentyp **nvarchar (max)** und hat den Standardwert NULL. Vom SQL Server-Agent wird eine Tokenersetzung bereitgestellt, die Ihnen beim Schreiben von Softwareprogrammen dieselbe Flexibilität wie Variablen bietet.  
...
`[ @additional_parameters = ] 'parameters'` [!INCLUDE[ssInternalOnly](../../includes/ssinternalonly-md.md)] *Parameters* ist vom Typ **ntext**und hat den Standardwert NULL.  
  
`[ @cmdexec_success_code = ] code` den von einem **CmdExec** -Subsystem zurückgegebenen Wert zurück, um anzugeben, dass der *Befehl* erfolgreich ausgeführt wurde. *Code* ist vom Datentyp **int**und hat den Standardwert **0**.  
...
`[ @proxy_name = ] 'proxy_name'` den Namen des Proxys, als der der Auftrags Schritt ausgeführt wird. *proxy_name* ist vom Datentyp **vom Datentyp sysname**und hat den Standardwert NULL. Wenn keine *proxy_id* angegeben wird, kein *proxy_name* angegeben wird und keine *user_name* angegeben ist, wird der Auftrags Schritt als Dienst Konto für [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]-Agent ausgeführt.  
