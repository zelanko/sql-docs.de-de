...
`[ @schedule_id = ] schedule_id` Der Bezeichner des zu ändernden Zeitplans. *Schedule_id* ist **Int**, hat keinen Standardwert. Entweder *Schedule_id* oder *Schedule_name* muss angegeben werden.  
  
`[ @name = ] 'schedule_name'` Der Name des zu ändernden Zeitplans. *Schedule_name*ist **Sysname**, hat keinen Standardwert. Entweder *Schedule_id* oder *Schedule_name* muss angegeben werden.  
  
`[ @new_name = ] new_name` Der neue Name für den Zeitplan. *New_name* ist **Sysname**, hat den Standardwert NULL. Wenn *New_name* NULL ist, der Namen des Zeitplans bleibt unverändert.  
...
`[ @freq_interval = ] freq_interval` Die Tage, die ein Auftrag ausgeführt wird. *Freq_interval* ist **Int**, hat den Standardwert **0**, und hängt vom Wert der *Freq_type*.  
...
`[ @freq_subday_interval = ] freq_subday_interval` Die Anzahl der *Freq_subday_type* -Perioden zwischen den einzelnen Ausführungen eines Auftrags. *Freq_subday_interval*ist **Int**, hat den Standardwert **0**.  
  
`[ @freq_relative_interval = ] freq_relative_interval` Vorhandensein eines Auftrags *Freq_interval* in jedem Monat, wenn *Freq_interval* ist **32** (mit relativem Monatsintervall). *Freq_relative_interval*ist **Int**, hat den Standardwert **0**, und kann einen der folgenden Werte sein.  
