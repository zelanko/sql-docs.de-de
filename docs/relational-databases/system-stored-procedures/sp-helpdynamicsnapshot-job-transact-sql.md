...
`[ @publication = ] 'publication'`Der Name der Veröffentlichung. *Publication* **%** ist vom **Datentyp vom Datentyp sysname**und hat den Standardwert, der Informationen zu allen gefilterten Daten Momentaufnahme-Aufträgen zurückgibt, die den angegebenen *dynamic_snapshot_jobid*und *dynamic_snapshot_jobname*für alle entsprechen. Veröffentlichungen.  
...
`[ @dynamic_snapshot_jobid = ] 'dynamic_snapshot_jobid'`Ein Bezeichner für einen Auftrag für eine Momentaufnahme gefilterter Daten. *dynamic_snapshot_jobid*ist vom Datentyp **uniqueidentifier**. der Standardwert ist NULL, womit alle Momentaufnahme Aufträge zurückgegeben werden, die mit der angegebenen *dynamic_snapshot_jobname*  
...
|**frequency_subday_type**|**int**|Der Typ, der definiert, wie oft der Agent ausgeführt wird, wenn *frequency_type* **4** (täglich) ist. die folgenden Werte sind möglich.<br /><br /> **1** = zum angegebenen Zeitpunkt<br /><br /> **2** = Sekunden<br /><br /> **4** = Minuten<br /><br /> **8** = Stunden|  
