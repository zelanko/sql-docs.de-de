...
|compress_snapshot|**bit**|Gibt an, dass die Momentaufnahme, die an den Speicherort *alt_snapshot_folder* geschrieben wird, in das [!INCLUDE[msCoName](../../includes/msconame-md.md)] CAB-Format komprimiert werden soll. der Wert **0** gibt an, dass die Momentaufnahme nicht komprimiert wird.|  
