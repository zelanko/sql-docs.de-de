...
|**fire_triggers_on_snapshot**|**true**|Replizierte Benutzertrigger werden ausgeführt, wenn die Anfangsmomentaufnahme angewendet wird.<br /><br /> Hinweis: Damit Trigger repliziert werden können, muss der Bitmasken Wert von *schema_option* den Wert **0x100**enthalten.|  
...
||**0x20000000**|Konvertiert große Objekt Datentypen (**nvarchar (max)** , **varchar (max)** und **varbinary (max)** ), die in [!INCLUDE[ssVersion2005](../../includes/ssversion2005-md.md)] eingeführt wurden, in Datentypen, die in unterstützt [!INCLUDE[ssVersion2000](../../includes/ssversion2000-md.md)]werden.|  
...
||**indizierter viewlogbased manualfilter**|Artikel für protokollbasierte indizierte Sicht mit manuell erstelltem Filter. Diese Option erfordert, dass die Eigenschaften *sync_object* und *Filter* ebenfalls festgelegt werden. Für diesen Artikeltyp muss die Basistabelle nicht separat veröffentlicht werden. Diese Option wird für Oracle-Verleger nicht unterstützt.|  
...
-   **upd_cmd**  
 ...
 Innerhalb einer vorhandenen Veröffentlichung können Sie **sp_changearticle** verwenden, um einen Artikel zu ändern, ohne die gesamte Veröffentlichung löschen und neu erstellen zu müssen.  
  
> [!NOTE]  
>  Beim Ändern des Werts von *schema_option*führt das System kein bitweises Update aus. Dies bedeutet, dass vorhandene Biteinstellungen beim Festlegen von *schema_option* mithilfe von **sp_changearticle**möglicherweise deaktiviert werden. Um die vorhandenen Einstellungen beizubehalten, führen Sie [| (Bitweises OR)](../../t-sql/language-elements/bitwise-or-transact-sql.md) zwischen dem Wert, den Sie festlegen, und dem aktuellen Wert von *schema_option*, der durch Ausführen von [sp_helparticle](../../relational-databases/system-stored-procedures/sp-helparticle-transact-sql.md)bestimmt werden kann.  
...
> [!NOTE]
>  Bei Veröffentlichungen mit verzögertem Update über eine Warteschlange muss der *schema_option* -Wert von **0x80** aktiviert werden. Die unterstützten *schema_option* -Werte für [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] nicht--Veröffentlichungen lauten: **0x01**, **0x02**, **0x10**, **0x40**, **0x80**, **0x1000** und **0x4000**.  
