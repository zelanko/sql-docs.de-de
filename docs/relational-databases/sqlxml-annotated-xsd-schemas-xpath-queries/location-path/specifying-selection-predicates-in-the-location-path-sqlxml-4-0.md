...
```  
 ...
 In dieser XPath-Abfrage sind `child` und `attribute` die Achsennamen. `Customer`ist der Knoten Test (true, `Customer` wenn ein ** \<Elementknoten>** ist, da ** \<Element>** der Haupt Knotentyp für `child` die Achse ist). 
  `attribute::CustomerID="ALFKI"` ist das Prädikat. Im `attribute` Prädikat ist die Achse und `CustomerID` ist der Knoten Test (true, wenn **CustomerID** ein Attribut des Kontext Knotens ist, da ** \<das Attribut>** der Haupt Knotentyp der **Attribut** Achse ist).  
