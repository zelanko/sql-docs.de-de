...
|**mirroring_failover_lsn**|**numerisch (25, 0)**|Protokollsequenznummer (Log Sequence Number, LSN) des aktuellsten Transaktionsprotokoll-Datensatzes, für den das Festschreiben auf der Festplatte auf beiden Partnern garantiert wird. Nach einem Failover wird der **mirroring_failover_lsn** von den Partnern als Punkt der Abstimmung verwendet, bei dem der neue Spiegel Server mit der Synchronisierung der neuen Spiegel Datenbank mit der neuen Prinzipal Datenbank beginnt.|  
...
|**mirroring_end_of_log_lsn**|**numerisch (25, 0)**|Das lokale Protokollende, das auf den Datenträger geleert wurde. Dies ist vergleichbar mit der festgeschriebenen LSN vom Spiegel Server (Weitere Informationen finden Sie in der **mirroring_failover_lsn** -Spalte).|  
|**mirroring_replication_lsn**|**numerisch (25, 0)**|Die maximale LSN, die von der Replikation gesendet werden kann.|  
