...
|**max_storage_size_mb**|**bigint**|Maximale Datenträger Größe für die Abfragespeicher in Megabyte (MB). Der Standardwert ist **100** MB.<br />Bei [!INCLUDE[sqldbesa](../../includes/sqldbesa-md.md)] Premium Edition beträgt der Standardwert 1 GB, und für [!INCLUDE[sqldbesa](../../includes/sqldbesa-md.md)] Basic Edition beträgt der Standardwert 10 MB.<br /><br /> Ändern Sie mit der `ALTER DATABASE <database> SET QUERY_STORE (MAX_STORAGE_SIZE_MB = <size>)`-Anweisung.|  
|**stale_query_threshold_days**|**bigint**|Anzahl von Tagen, in denen Abfragen ohne Richtlinien Einstellungen in Abfragespeicher aufbewahrt werden. Der Standardwert ist **30**. Legen Sie auf 0 fest, um die Aufbewahrungs Richtlinie zu deaktivieren.<br />Für die Basic-Edition von [!INCLUDE[sqldbesa](../../includes/sqldbesa-md.md)] ist der Standardwert 7 Tage.<br /><br /> Ändern Sie mit der `ALTER DATABASE <database> SET QUERY_STORE ( CLEANUP_POLICY = ( STALE_QUERY_THRESHOLD_DAYS = <value> ) )`-Anweisung.|  
|**max_plans_per_query**|**bigint**|Begrenzt die maximale Anzahl gespeicherter Pläne. Der Standardwert ist **200**. Wenn der Höchstwert erreicht ist, beendet Abfragespeicher die Erfassung neuer Pläne für diese Abfrage. Wenn Sie auf 0 festlegen, wird die Einschränkung in Bezug auf die Anzahl der aufgezeichneten Pläne entfernt.<br /><br /> Ändern Sie mit der `ALTER DATABASE<database> SET QUERY_STORE (MAX_PLANS_PER_QUERY = <n>)`-Anweisung.|  
...
|**query_capture_mode_desc**|**nvarchar(60)**|Textbeschreibung des tatsächlichen Aufzeichnungsmodus Abfragespeicher:<br /><br /> Alle (Standardeinstellung für [!INCLUDE[ssSQL15](../../includes/sssql15-md.md)])<br /><br /> **Auto** (Standardwert für [!INCLUDE[sqldbesa](../../includes/sqldbesa-md.md)])<br /><br /> NONE|  
