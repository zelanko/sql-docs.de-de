...
|**ExtractApplicationScopedObjectsOnly={True &#124; False}**|**True**|Beim Wert **True** werden nur Objekte im Anwendungsbereich aus der Quelle extrahiert. Beim Wert **False** werden sowohl Objekte im Anwendungsbereich als auch Objekte außerhalb des Anwendungsbereichs extrahiert.|  
|**ExtractReferencedServerScopedElements={True &#124; False}**|**True**|Beim Wert **True** werden Anmelde-, Serverüberwachungs- und Anmeldeinformationsobjekte extrahiert, auf die von Quelldatenbankobjekten verwiesen wird.|  
...
|**BlockOnPossibleDataLoss={ True &#124; False}**|**True**|Gibt an, ob die Veröffentlichungssequenz beendet wird, wenn durch den Veröffentlichungsvorgang ein Datenverlust verursacht werden könnte.|  
|**BlockWhenDriftDetected={ True &#124; False}**|**True**|Gibt an, ob die Aktualisierung einer Datenbank, deren Schema nicht mehr mit der Registrierung übereinstimmt oder aus der Registrierung entfernt wurde, blockiert wird.|  
...
|**DisableAndReenableDdlTriggers={True &#124; False}**|**True**|Gibt an, ob DDL-Trigger (Data Definition Language) am Anfang des Veröffentlichungsprozesses deaktiviert und am Ende der Veröffentlichungsaktion erneut aktiviert werden.|  
|**DoNotAlterChangeDataCaptureObjects={ True &#124; False}**|**True**|Beim Wert **True** werden Change Data Capture-Objekte nicht geändert.|  
|**DoNotAlterReplicatedObjects=( True &#124; False}**|**True**|Gibt an, ob replizierte Objekte während der Überprüfung identifiziert werden.|  
|**DropConstraintsNotInSource= {True &#124; False}**|**True**|Gibt an, ob durch die Veröffentlichungsaktion Einschränkungen, die in der Datenbankmomentaufnahme (DACPAC-Datei) nicht vorhanden sind, bei der Veröffentlichung in einer Datenbank aus der Zieldatenbank gelöscht werden.|  
|**DropDmlTriggersNotInSource= {True &#124; False}**|**True**|Gibt an, ob durch die Veröffentlichungsaktion DML-Trigger (Data Manipulation Language), die in der Datenbankmomentaufnahme (DACPAC-Datei) nicht vorhanden sind, bei der Veröffentlichung in einer Datenbank aus der Zieldatenbank gelöscht werden.|  
|**DropExtendedPropertiesNotInSource= {True &#124; False}**|**True**|Gibt an, ob durch die Veröffentlichungsaktion erweiterte Eigenschaften, die in der Datenbankmomentaufnahme (DACPAC-Datei) nicht vorhanden sind, bei der Veröffentlichung in einer Datenbank aus der Zieldatenbank gelöscht werden.|  
|**DropIndexesNotInSource= {True &#124; False}**|**True**|Gibt an, ob durch die Veröffentlichungsaktion Indizes, die in der Datenbankmomentaufnahme (DACPAC-Datei) nicht vorhanden sind, bei der Veröffentlichung in einer Datenbank aus der Zieldatenbank gelöscht werden.|  
...
|**IgnoreCryptographicProviderFile= {True &#124; False}**|**True**|Gibt an, ob Unterschiede im Dateipfad für einen kryptografischen Anbieter beim Veröffentlichen in einer Datenbank ignoriert oder aktualisiert werden.|  
...
|**IgnoreFileAndLogFilePath={True &#124; False}**|**True**|Gibt an, ob Unterschiede im Datei- und Protokolldateipfad beim Veröffentlichen in einer Datenbank ignoriert oder aktualisiert werden.|  
|**IgnoreFilegroupPlacement= {True &#124; False}**|**True**|Gibt an, ob Unterschiede in der **FILEGROUP**-Platzierung beim Veröffentlichen in einer Datenbank ignoriert oder aktualisiert werden.|  
|**IgnoreFileSize= {True &#124; False}**|**True**|Gibt an, ob Unterschiede in der Dateigröße beim Veröffentlichen in einer Datenbank ignoriert oder aktualisiert werden.|  
|**IgnoreFillFactor= {True &#124; False}**|**True**|Gibt an, ob Unterschiede in den Füllfaktoren beim Veröffentlichen in einer Datenbank ignoriert oder aktualisiert werden.|  
  
|Eigenschaft|Standard|BESCHREIBUNG|  
|------------|-----------|---------------|  
|**IgnoreFullTextCatalogFilePath= {True &#124; False}**|**True**|Gibt an, ob Unterschiede im Pfad zu Volltext-Indexdateien beim Veröffentlichen in einer Datenbank ignoriert oder aktualisiert werden.|  
...
|**IgnoreIndexPadding= {True &#124; False}**|**True**|Gibt an, ob Unterschiede im Auffüllen von Indizes beim Veröffentlichen in einer Datenbank ignoriert oder aktualisiert werden.|  
|**IgnoreKeywordCasing= {True &#124; False}**|**True**|Gibt an, ob Unterschiede in der Groß-/Kleinschreibung für Schlüsselwörter beim Veröffentlichen in einer Datenbank ignoriert oder aktualisiert werden.|  
...
|**IgnoreLoginSids= {True &#124; False}**|**True**|Gibt an, ob Unterschiede in der Sicherheits-ID (SID) beim Veröffentlichen in einer Datenbank ignoriert oder aktualisiert werden.|  
...
|**IgnoreObjectPlacementOnPartitionScheme= {True &#124; False}**|**True**|Gibt an, ob die Platzierung eines Objekts in einem Partitionsschema beim Veröffentlichen in einer Datenbank ignoriert oder aktualisiert wird.|  
...
|**IgnoreRouteLifetime= {True &#124; False}**|**True**|Gibt an, ob Unterschiede in den Rollenmitgliedschaften von Anmeldenamen beim Veröffentlichen in einer Datenbank ignoriert oder aktualisiert werden.|  
|**IgnoreSemicolonBetweenState= {True &#124; False}**|**True**|Gibt an, ob Unterschiede hinsichtlich Semikolons zwischen Transact-SQL-Anweisungen beim Veröffentlichen in einer Datenbank ignoriert oder aktualisiert werden.|  
...
|**IgnoreWhitespace= {True &#124; False}**|**True**|Gibt an, ob Unterschiede in Leerzeichen beim Veröffentlichen in einer Datenbank ignoriert oder aktualisiert werden.|  
...
|**PopulateFilesOnFilegroups= {True &#124; False}**|**True**|Gibt an, ob beim Erstellen einer neuen **FileGroup** in der Zieldatenbank ebenfalls eine neue Datei erstellt werden soll.|  
...
|**ScriptRefreshModule= {True &#124; False}**|**True**|Gibt an, ob am Ende des Veröffentlichungsskripts Aktualisierungsanweisungen eingeschlossen werden.|  
...
|**UnmodifiableObjectWarnings= {True &#124; False}**|**True**|Gibt an, ob Warnungen generiert werden, wenn Unterschiede in Objekten gefunden werden, die nicht änderbar sind (z. B. wenn die Dateigröße oder Dateipfade für eine Datei unterschiedlich sind).|  
|**VerifyCollationCompatibility={True &#124; False}**|**True**|Gibt an, ob die Kompatibilität von Sortierungen überprüft wird.|  
|**VerifyDeployment={True &#124; False}**|**True**|Gibt an, ob vor der Veröffentlichung Überprüfungen ausgeführt werden sollen, durch die die Veröffentlichungsaktion beendet wird, wenn Probleme vorliegen, die eine erfolgreiche Veröffentlichung blockieren könnten. Die Veröffentlichungsaktion könnte beispielsweise beendet werden, wenn während der Veröffentlichung Fehler auftreten, weil Fremdschlüssel aus der Zieldatenbank nicht im Datenprojekt vorhanden sind.|  
