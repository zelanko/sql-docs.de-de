...
|SQL_ATTR_ROWS_FETCHED_PTR FEST|Legt die Adresse des Puffers, in dem **SQLFetch** und **SQLFetchScroll** geben die Anzahl der abgerufenen Zeilen zurück. Wenn **SQLExtendedFetch** wird aufgerufen, diesen Puffer nicht ausgefüllt ist aber die *RowCountPtr* -Argument zeigt auf die Anzahl der abgerufenen Zeilen.|  
|SQL_ATTR_ROW_ARRAY_SIZE|Legt die Größe des Rowsets, die von verwendet **SQLFetch** und **SQLFetchScroll**.|  
|SQL_ROWSET_SIZE SETZEN|Legt die Größe des Rowsets, die von verwendet **SQLExtendedFetch**. ODBC 3.*.x* Treiber implementieren Sie dies, wenn sie mit ODBC 2. arbeiten möchten. *X* Anwendungen, die Aufrufen **SQLExtendedFetch** oder **SQLSetPos**.|  
