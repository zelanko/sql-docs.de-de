---
title: Funktionseinschränkungen festlegen | Microsoft Docs
ms.custom: ''
ms.date: 01/19/2017
ms.prod: sql
ms.prod_service: connectivity
ms.reviewer: ''
ms.technology: connectivity
ms.topic: conceptual
helpviewer_keywords:
- ODBC desktop database drivers [ODBC]
- desktop database drivers [ODBC]
ms.assetid: 18e7a0fd-cc5d-4109-b542-cac6893a29c3
author: David-Engel
ms.author: v-daenge
ms.openlocfilehash: 76f679a007800ecf4f1c889d7cfe2b9746deb708
ms.sourcegitcommit: ce94c2ad7a50945481172782c270b5b0206e61de
ms.translationtype: MT
ms.contentlocale: de-DE
ms.lasthandoff: 04/14/2020
ms.locfileid: "81300850"
---
# <a name="set-functions-limitations"></a>Einschränkungen für Set-Funktionen
Die set-Funktionen (AVG, MAX, MIN und SUM) unterstützen das Schlüsselwort DISTINCT nicht.
